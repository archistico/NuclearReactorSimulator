...
M0 through M9 are validated, with M7, M8 and M9 gates complete. **M10.1–M10.9.4 are VALIDATED** and M10.9.4 remains the official milestone baseline. The user supplied a consolidated `M10.9.4.1_nuova` continuation base containing the accumulated A/B/C hardening and explicitly asked to continue from the Phase-D boundary. **M10.9.4.1-D.1 is implemented with local validation pending; D.2 authority evidence is now the active candidate**: current-v2 turbine admission is vapor-mass-fraction limited so liquid cannot cross the stage as a zero-work bypass, while historical definitions retain unrestricted legacy semantics. The previously observed 300-second wall-clock performance-budget overrun remains tracked as performance/numerical-hardening evidence and is not silently erased by D.1. D.2 changes no production physics: it freezes the current-v2 resistance budget and adds deterministic analytical plus ±10 rpm runtime authority evidence before any resistance/Stodola/effective-area decision.
...
| M10 | IN PROGRESS | M10.1–M10.9.4 validated; user-supplied consolidated M10.9.4.1 A/B/C continuation base accepted for ongoing development; D.1 turbine admission phase-policy closure is implemented with local validation pending; D.2 turbine admission authority evidence is the active candidate; 300 s wall-clock performance evidence remains tracked; M10.9.4.1 completes before M10.9.5; M10 closes at M10.9.8 |
...
**Continuation note:** M8.1–M8.7, M9.1–M9.7 and M10.1–M10.9.4 are validated. M10.9.4 remains the official milestone baseline. The user supplied a consolidated M10.9.4.1 A/B/C continuation tree after additional local work and explicitly resumed development at Phase D. D.1 turbine admission phase-policy closure is implemented with local validation pending; D.2 turbine admission authority evidence is the active candidate. The earlier 300-second wall-clock performance-budget overrun remains tracked for the later numerical/performance gate; see `PROJECT_HANDOFF.md`, `NEW_CHAT_START.md`, `milestones/M10.9.4.1.md` and `OPERATIONAL_ENVELOPE_NUMERICAL_HARDENING_PLAN.md`.
