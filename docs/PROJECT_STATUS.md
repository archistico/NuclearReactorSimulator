...
M0 through M9 are validated, with M7, M8 and M9 gates complete. **M10.1–M10.9.4 are VALIDATED**. The user-confirmed final manual HMI / engineering-schematic checklist promotes **M10.9.4 — Subsystem Engineering Schematics** as the official milestone baseline. **M10.9.4.1-A — Extended Operating-Envelope Audit** has been executed without changing production physics: compilation and the ordinary suite pass, but the explicit healthy-reference journey trips near 70 simulated seconds. Phase A is not validated. **M10.9.4.1-A.2 Hotfix 1 — Condenser Capacity Headroom** is the current candidate; A.1 evidence completion is included and local validation is pending. Later physical/numerical hardening remains owned by phases B–I.
...
| M10 | IN PROGRESS | M10.1–M10.9.4 validated; M10.9.4.1-A non-green at ~70 s; A.2 condenser-headroom candidate pending validation; M10.9.4.1 completes before M10.9.5; M10 closes at M10.9.8 |
...
### Current M10.9.4.1-A audit checkpoint

Hotfix 20 Fix 2, Hotfix 21, Hotfix 22 and Hotfix 23 are **VALIDATED STRUCTURAL CHECKPOINTS**, and the final manual checklist promoted M10.9.4 to the official validated baseline. M10.9.4.1-A deliberately leaves production physics unchanged. Its build and ordinary suite pass, but the explicit 300-second reference journey fails at step 7000 (~70 s) with turbine/generator trip latched. Conservation remains closed. The combined action signature with bounded speed/frequency identifies `condenser-high-backpressure`; A.2 Hotfix 1 adds one-second direct evidence and current-v2 capacity headroom while leaving the solver and threshold unchanged.
...
**Continuation note:** M8.1–M8.7, M9.1–M9.7 and M10.1–M10.9.4 are validated. M10.9.4 is the official milestone baseline; M10.9.4.1-A has executed and exposed a trip near 70 simulated seconds, with A.2 Hotfix 1 validation pending; see `PROJECT_HANDOFF.md`, `NEW_CHAT_START.md`, `milestones/M10.9.4.md`, `M10_9_4_FINAL_MANUAL_VALIDATION_CHECKLIST.md`, `milestones/M10.9.4.1.md` and `OPERATIONAL_ENVELOPE_NUMERICAL_HARDENING_PLAN.md`.
