...
M0 through M9 are validated, with M7, M8 and M9 gates complete. **M10.1–M10.9.4 are VALIDATED** and M10.9.4 remains the official milestone baseline. The user supplied a consolidated `M10.9.4.1_nuova` continuation base containing the accumulated A/B/C hardening and explicitly asked to continue from the Phase-D boundary. **M10.9.4.1-D.1 and D.2 Hotfix 1 are locally validated; D.3 governor/actuator tracking evidence is locally validated and closes without tracking anti-windup; E.1 accepted the 10 MWe reduced-scale educational target and E.2 Hotfix 1 is now the active coordinated runtime-migration candidate**: current-v2 turbine admission is vapor-mass-fraction limited so liquid cannot cross the stage as a zero-work bypass, while historical definitions retain unrestricted legacy semantics. The previously observed 300-second wall-clock performance-budget overrun remains tracked as performance/numerical-hardening evidence and is not silently erased by D.1. D.2 changes no production physics: it freezes the current-v2 resistance budget and adds deterministic analytical plus ±10 rpm runtime authority evidence before any resistance/Stodola/effective-area decision.
...
| M10 | IN PROGRESS | M10.1–M10.9.4 validated; user-supplied consolidated M10.9.4.1 A/B/C continuation base accepted for ongoing development; D.1 turbine admission phase-policy closure and D.2 Hotfix 1 authority evidence are locally validated; D.3 governor/actuator tracking evidence is locally validated; E.1 is accepted and E.2 Hotfix 1 is the active runtime-migration candidate; 300 s wall-clock performance evidence remains tracked; M10.9.4.1 completes before M10.9.5; M10 closes at M10.9.8 |
...
User validation is green for the exact 300-second sustained journey (2m 07s), the explicit 60-second synchronization journey, build (0 warnings / 0 errors) and the ordinary suite (895 passed, 11 explicit skipped, 0 failed). A.3 scale evidence remains an open design contract rather than an implemented scale migration. Phase B has progressed through locally validated B.1 liquid-inventory closure and B.2 drum-source closure. B.3 now adds explicit low-inventory/separation diagnostics and measured low-level/low-low protection without changing those validated source laws.
...
**Continuation note:** M8.1–M8.7, M9.1–M9.7 and M10.1–M10.9.4 are validated. M10.9.4 remains the official milestone baseline. The user supplied a consolidated M10.9.4.1 A/B/C continuation tree after additional local work and explicitly resumed development at Phase D. D.1 turbine admission phase-policy closure and D.2 Hotfix 1 authority evidence are locally validated; D.3 governor/actuator tracking evidence is locally validated; E.1 is accepted and E.2 Hotfix 1 is the active runtime-migration candidate. The earlier 300-second wall-clock performance-budget overrun remains tracked for the later numerical/performance gate; see `PROJECT_HANDOFF.md`, `NEW_CHAT_START.md`, `milestones/M10.9.4.1.md` and `OPERATIONAL_ENVELOPE_NUMERICAL_HARDENING_PLAN.md`.
...
M10 is **IN PROGRESS** as **Operator Computer, Supervisory Automation & Human-Machine Integration**. M10.1–M10.9.4 are validated, M10.9.4 is the current official baseline. The user-supplied consolidated M10.9.4.1 continuation base carries the accumulated A/B/C hardening and Phase D is now locally validated and closed; E.1 accepts the 10 MWe reduced-scale target and E.2 Hotfix 1 now implements the coordinated current-v2 runtime migration plus the signed generator-to-rotor torque seam fix; local validation is pending. The historical 300-second wall-clock budget observation remains tracked for later numerical/performance hardening. M10.9.4.1 remains before M10.9.5 for operational-envelope and numerical hardening. The approved sequence then continues through command consequences and deterministic performance-oriented training. The operator computer remains an Application/App aggregation surface; real Manual / Assisted / Supervisory Automatic plant control remains canonical M5 ownership. Training assistance (`TrainingGuidanceMode`) remains a separate independent axis.
...
## Validated M9 gate / M10.1–M10.9.4 validated / B.1–B.3+C.1–C.2 locally validated / current M10.9.4.1-C.2 Hotfix 2 candidate

M8.1–M8.7 are validated and the M8 gate is complete. M9.1–M9.7 are validated and the M9 gate is complete; M9.7 remains the validated M9 phase-gate baseline. The user confirmed 760/760 automated tests passed after hotfix 5, and the final corrected `MainWindow.axaml` is integrated. **M10.1–M10.9.4 are VALIDATED**. The user explicitly confirmed the M10.9.4 final manual checklist after all automated and explicit long-running gates passed. **M10.9.4 is the current validated application baseline**. **The M10.9.4.1-A long-horizon failure was traced to the current-v2 operating seed and corrected. The user later supplied a consolidated A/B/C continuation tree and requested Phase-D continuation; D.1 and D.2 Hotfix 1 are locally validated; D.3 governor/actuator tracking evidence is locally validated and closes without tracking anti-windup; E.1 accepts the 10 MWe reduced-scale educational target; E.2 Hotfix 1 applies that target to the current-v2 runtime and fixes the signed motoring torque seam; local validation is pending. The earlier 300-second wall-clock budget observation remains tracked as performance evidence**; M10 closes only after M10.9.8 integrated human-automation-HMI validation.

## M10.9.4.1-D.3 current audit gate

D.1 admission phase-policy closure and D.2 Hotfix 1 authority evidence are locally validated. D.3 is locally validated: the audit exercised 23.418 percentage points of command/position lag and measured only 0.134 percentage points of integral excursion, below the 2 pp correction gate. No D.3.1 tracking anti-windup is introduced. E.1 accepts the 10 MWe reduced-scale target; E.2 implements the coordinated migration and remains pending local validation.
