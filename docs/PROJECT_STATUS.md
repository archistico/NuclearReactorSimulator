> **Current validated continuation:** M10.9.4.1-H.17 Hotfix 6; Phase G complete and current-v2 production safely remains explicit at 10 ms. **Current working source:** M10.9.4.1-H.18 CANDIDATE — turbine-inlet continuity extension and residual-floor split diagnosis.
...
M0 through M9 are validated, with M7, M8 and M9 gates complete. **M10.1–M10.9.4 and the cumulative M10.9.4.1-H.17 Hotfix 6 continuation are VALIDATED; Phase G is complete and production current-v2 remains explicit.** H.18 is the current shadow-only turbine-inlet continuity / residual-floor split candidate. The H.4-selected hybrid method remains non-authoritative numerical evidence only.

Approved future product direction remains frozen in `FUTURE_GAMEPLAY_CONTROL_ROOM_AND_ACCIDENT_DIRECTION.md` plus ADR 0121–0123. These decisions do **not** change the immediate numerical-hardening sequence. H.10–H.12 localized the original failures to inverse thermodynamic branch selection; H.13 Hotfix 2 validated bounded 2%/5 K continuity at `steam|stop-out`; H.15 localized interval 723 to the same mechanism at `header`; H.16 validated `steam|stop-out|header` at 15/15. H.17 Hotfix 6 is now validated diagnostic evidence over 30,000 intervals/four profiles: 3,046 P060/F040 trigger intervals, 92 episodes, 473 representatives, 228/473 convergence and 245 failures. The H.17 all-node scan discovered `turbine-inlet`; 120 failures show candidate-vs-explicit turbine-inlet phase mismatch while 125 do not. H.18 is the current split-diagnosis candidate: it evaluates all 245 failures with unchanged H.9 and unchanged 2%/5 K hysteresis extended only to `steam|stop-out|header|turbine-inlet`, then diagnoses any remaining failures for fixed-point residual floor or additional branch disagreement. Phase I remains deferred.
...
| M10 | IN PROGRESS | M10.1–M10.9.4 and M10.9.4.1 through H.17 Hotfix 6 validated; Phase G complete; H.18 turbine-inlet continuity / residual-floor split candidate prepared; M10 closes at M10.9.8 |
...
**Continuation note:** M8.1–M8.7, M9.1–M9.7, M10.1–M10.9.4 and M10.9.4.1-H.17 Hotfix 6 are validated; Phase G is complete and production current-v2 remains explicit. H.10–H.16 established the inverse-branch continuity mechanism and the three-node 2%/5 K shadow policy. H.17 validated the long-horizon diagnostic infrastructure but found only 228/473 convergence and split 245 failures into 120 `turbine-inlet` mismatch cases plus 125 non-mismatch cases. The current working source H.18 evaluates that split without changing production, H.9, P060/F040 or hysteresis limits. See `PROJECT_HANDOFF.md`, `NEW_CHAT_START.md`, `M10_9_4_1_H18_TURBINE_INLET_CONTINUITY_RESIDUAL_FLOOR_SPLIT_DIAGNOSIS.md`, `milestones/M10.9.4.1.md` and `OPERATIONAL_ENVELOPE_NUMERICAL_HARDENING_PLAN.md`.
