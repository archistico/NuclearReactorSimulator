> **Current continuation candidate:** M10.9.4.1-D.3.2 Hotfix 3 preserves D.3.1 passive rotor loss, D.3.2 admission-train isolation and the uniform PLANT renderer; it keeps the loaded 28% bias and 276.7 °C stop-out seed while changing only the loaded main-steam-line resistance from 1,000 to 850 Pa·s²/kg². Synchronization remains at 1,000 Pa·s²/kg².

# Project Status

M0 through M9 are validated, with M7, M8 and M9 gates complete. **M10.1–M10.9.4 are VALIDATED** and M10.9.4 remains the official milestone baseline. The user supplied a consolidated `M10.9.4.1_nuova` continuation base containing the accumulated A/B/C hardening and explicitly asked to continue from the Phase-D boundary. **M10.9.4.1-D.1, D.2 and D.2 Hotfix 1 are locally user-validated. D.3 evidence proved that the breaker-open rotor had no passive deceleration path; D.3.2 Hotfix 3 is now the active current-v2 candidate.** Local evidence rejected the bias-only Hotfix 1, Hotfix 2 corrected the stop pressure grade, and Hotfix 3 addresses the remaining loaded main-steam-line bottleneck by changing only that profile from 1,000 to 850 Pa·s²/kg². The synchronization profile remains at 1,000 Pa·s²/kg². The previously observed 300-second wall-clock performance-budget overrun remains tracked separately. PID/PI gains, actuator travel, droop, valve/stage resistances, generator scale, protections and legacy behavior remain unchanged pending local validation and the later Phase-E scale decision.
...
| M10 | IN PROGRESS | M10.1–M10.9.4 validated; user-supplied consolidated M10.9.4.1 A/B/C continuation base accepted; D.1, D.2 and D.2 Hotfix 1 locally user-validated; D.3.2 Hotfix 3 loaded desktop main-steam capacity rebalance active; 300 s wall-clock performance evidence remains tracked; M10.9.4.1 completes before M10.9.5; M10 closes at M10.9.8 |
...
**Continuation note:** M8.1–M8.7, M9.1–M9.7 and M10.1–M10.9.4 are validated. M10.9.4 remains the official milestone baseline. The user supplied a consolidated M10.9.4.1 A/B/C continuation tree and resumed Phase D. D.1, D.2 and D.2 Hotfix 1 are locally user-validated. D.3.2 Hotfix 3 loaded desktop main-steam capacity rebalance is the active candidate after D.3 exposed indefinite zero-torque coasting. The earlier 300-second wall-clock performance-budget overrun remains tracked for the later numerical/performance gate; see `PROJECT_HANDOFF.md`, `NEW_CHAT_START.md`, `milestones/M10.9.4.1.md` and `OPERATIONAL_ENVELOPE_NUMERICAL_HARDENING_PLAN.md`.
