...
Hotfix 2 replaces that implementation with a monotonic internal observation version. `Observe(...)` increments the version when the immutable `ChallengeConditionObservation` value changes. Because the observation includes logical step/evidence, an active condition sampled on successive deterministic steps normally still advances the version. This is intentional preservation of the pre-Hotfix-2 `LifecycleChanged` behavior; Hotfix 2 removes fingerprint materialization cost, it does not redefine notifications as outcome-only. `Evaluate(...)` compares the version before/after evaluation plus the existing transition-count check. The public lifecycle snapshot and `LifecycleChanged` contract do not expose the version. Any later notification-semantic split must be measured and explicitly designed in M11.3.
