...
Accepted target / M10.9.4.1-E.1. Production migration deferred to E.2.
...
The reference current-v2 educational plant will migrate to a nominal **10 MWe generator scale**. The accepted scale basis retains:
...
E.1 records the target only. E.2 must apply the migration as one coordinated physical change covering nameplate, governor normalization, signed/bidirectional grid coupling, HMI ranges and trajectory evidence. The current 150 rpm droop rise and 10 MW coupling values are not automatically accepted as migrated values.
...
- Reverse-power, underfrequency and loss-of-synchronism protections remain deferred until signed generator/grid states are physically implemented and validated.
