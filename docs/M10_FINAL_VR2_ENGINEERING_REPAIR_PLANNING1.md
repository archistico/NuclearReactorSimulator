...
**Status:** VALIDATED / CLOSED — RP1A/RP1B/C4 qualification, runtime attribution, A2, FDPC2 and RP1C Selection 1 are returned/adjudicated; `SELECT-C4` is recorded; the only live successor is planning-only `R1-IMPLEMENTATION-PLANNING1`; production implementation/activation and downstream VR3/P3-R1/second replacement-long authority remain unauthorized  
...
That historical Refinement-4 decision remains frozen provenance. Its successor chain is now complete: Performance Measurement Replanning 1 -> Refinement 5 -> C4 Planning 1 REV2 -> C4 Test-Only Implementation 1. Returned C4 evidence is qualified as `C4-QUALIFIED-ALLOCATION-TAIL-CLOSED`.

The `409.30666666666673 us` strict single-call maximum was never relaxed or erased. C4 closes the R1 qualification contract with zero strict-ceiling exceedances, but remains test/reference/shadow evidence until RP1C makes a separate selection decision.

ADR 0195 remains the historical measurement/selection boundary; the returned C4 adjudication is the current authority for advancing to RP1C planning.
...
The returned Planning 1 audit is reviewed `PASS-AS-AUTHORED`, and returned RP1A REV1 evidence is VALIDATED. First-generation RP1B, Refinement 1 C2/D2 and Refinement 2 C3/D3 are validated evidence. Refinement 2 produced physically complete C3/D3 results, but C3 requires performance-tail attribution and D3's recorded eligibility omitted seam worst-case timing. RP1B Refinement 3 subsequently returned complete timing evidence: exact-v9 and targeted C3 calls are below ceiling, while one R1-side seam call reached 3667.1 us. RP1B Refinement 4 is validated returned localization evidence. Performance Measurement Replanning 1 is VALIDATED / CLOSED and its five-process rule fired on returned Refinement 5 evidence. Refinement 5 is VALIDATED as `C3-SAME-BOUNDARY-SLOW-PATH-CONFIRMED`; C4 Planning 1 REV2 and C4 Test-Only Implementation 1 are now returned/adjudicated PASS, with C4 classified `C4-QUALIFIED-ALLOCATION-TAIL-CLOSED`. Historical checkpoint — RP1C Planning 1 was the active planning candidate at that stage; it was later superseded by returned FDPC2, Selection Planning 1 and Selection 1.
...
Refinement 5 returned `C3-SAME-BOUNDARY-SLOW-PATH-CONFIRMED`; C4 Planning 1 REV2 then authorized the separately versioned test-only C4 evidence candidate. Returned C4 execution is now independently adjudicated PASS as `C4-QUALIFIED-ALLOCATION-TAIL-CLOSED`, with bit-equivalent semantics, zero measured allocation/fallback and zero strict-ceiling exceedances. Historical checkpoint — the next authorized action at that stage was **RP1C Planning 1 only**. That planning candidate froze the corrected full selection predicate, C4/D3 readiness reconstruction, mandatory `NO-SELECTION` option and the remaining immutable-C4 full-domain performance confirmation; it is superseded by returned FDPC2 and RP1C Selection 1.


## 11. Runtime-sensitive branch update — 2026-09-17

Returned Runtime Factor Isolation 1 evidence is accepted as complete. The single-factor A↔B intervention establishes that enabling TieredCompilation is materially causal for the gross exact-v9 wall-clock slowdown, reducing the aggregate median from about 109.0 us to 4.6 us and strict exceedances from 20 to 1 while the other frozen compilation variables remain fixed. B↔C gives only a small central QuickJit effect and one isolated strict-tail difference; C↔D gives no material QuickJitForLoops tail benefit. This does not reinterpret Full-Domain Performance Confirmation 1 and does not make C4 selection-ready.

Dynamic PGO Comparator 1 has now returned and is adjudicated from the complete 46-file / 230,400-call evidence tree. PGO ON materially and repeatedly improves the central exact-v9 timing distribution under QJFL ON, but the sparse 1→0 strict-exceedance contrast does not establish rare-tail causality. Historical checkpoint — the successor at that stage was planning-only `RP1C-C4-RUNTIME-CONFIGURATION-IMPACT-ASSESSMENT-PLANNING1`. Planning 1, Host-Provenance Amendment 1 and Frozen Evidence Ordinary Compaction 1 have since returned `PASS-AS-AUTHORED`. Historical checkpoint — the live successor at that stage was evidence-only `RP1C-C4-RUNTIME-CONFIGURATION-IMPACT-ASSESSMENT1`; it is superseded by returned A2, FDPC2 and RP1C Selection 1.


### Historical returned FDPC2 / RP1C Selection Planning 1 (superseded)

Returned FDPC2 is adjudicated `C4-FULL-DOMAIN-PERFORMANCE-CONFIRMED`. C4 under `AMBIENT-UNSET` is now the sole selection-ready pair under the corrected predicate; D3 remains blocked by the historical seam maximum. At that checkpoint the live successor was planning-only `RP1C-ENGINEERING-REPAIR-SELECTION-PLANNING1`, which preserved the mandatory `SELECT-NONE` option and grants no production authority.

### Returned Selection Planning 1 / RP1C Selection 1

Historical checkpoint (superseded by returned Selection 1): Selection Planning 1 returned `PASS-AS-AUTHORED`. At that checkpoint the live successor was decision-only `RP1C-ENGINEERING-REPAIR-SELECTION1`, with exact decision space `SELECT-C4 | SELECT-NONE`. The authored decision is `SELECT-C4`; it is supported by green FDPC2 and preserved D3 blocking evidence, but it grants no production repair authority. Returned selection adjudication must occur before `R1-IMPLEMENTATION-PLANNING-ONLY`.

## 12. RP1C selection returned -- R1 implementation planning checkpoint

Returned RP1C selection records `SELECT-C4` and is adjudicated complete. The selected candidate remains test/reference/shadow evidence until the separately returned R1 planning gate authorizes bounded production staging. The live successor is `R1-IMPLEMENTATION-PLANNING1` only. Existing closure modes, exact-v9 and production defaults remain unchanged.
