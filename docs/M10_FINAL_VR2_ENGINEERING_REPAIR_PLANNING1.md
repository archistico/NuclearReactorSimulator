...
**Status:** VALIDATED / CLOSED — returned static audit `PASS-AS-AUTHORED`; RP1A REV1, first-generation RP1B, Refinements 1–5 and C4 Test-Only Implementation 1 are VALIDATED returned evidence; C4 is adjudicated `C4-QUALIFIED-ALLOCATION-TAIL-CLOSED`; RP1C Planning 1 is the active planning candidate; RP1C selection and production repair remain unauthorized and C4 still requires full-domain performance confirmation  
...
Refinement 5 returned `C3-SAME-BOUNDARY-SLOW-PATH-CONFIRMED`; C4 Planning 1 REV2 then authorized the separately versioned test-only C4 evidence candidate. Returned C4 execution is now independently adjudicated PASS as `C4-QUALIFIED-ALLOCATION-TAIL-CLOSED`, with bit-equivalent semantics, zero measured allocation/fallback and zero strict-ceiling exceedances. The next authorized action is therefore **RP1C Planning 1 only**. The planning candidate freezes the corrected full selection predicate, C4/D3 readiness reconstruction, mandatory `NO-SELECTION` option and the remaining immutable-C4 full-domain performance confirmation. RP1C selection and production thermodynamic changes remain unauthorized until Planning 1 is returned/adjudicated and the separately authorized confirmation closes that evidence gap.
