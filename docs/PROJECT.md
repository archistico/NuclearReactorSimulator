...
**Active candidate: M10 Final Replacement-Long Closure Plan 1 — P1A ASYMPTOTIC CLOSURE EXTENSION CANDIDATE.**
...
P1A now executes that amendment: exact-v9 5.5/6 MWe only, unchanged frozen P1 calibration/tolerances, deterministic reproduction of the returned P1 checkpoints, and a hard 3,600 s total hold ceiling. exact-v4 is not rerun. P1A returns only to P2R; it cannot authorize P3 directly. A second replacement-long baseline remains forbidden until P4 short 5→10→5 qualification passes.
...
```bat
scripts\run-m10-final-replacement-long-closure-plan1-p1a-asymptotic-closure-extension.cmd
```

P1A first executes the ordinary Release gate, then the explicit bounded extension. It must reproduce the frozen P1 exact-v9 checkpoints before entering new simulated time, may run only the 5.5 and 6 MWe exact-v9 probes, and stops no later than 3,600 s after each load command. Returned P1A evidence must go to P2R before any P3 branch is authorized.
...
- Phase I is closed; exact-v4 remains validated historical production evidence, exact-v9 is now authoritative production, and the final long gate remains open under Closure Plan 1 pending P1A/P2R classification, the evidence-selected P3 branch, P4 short qualification, P5 baseline-2 freeze/execution and P6 closure; the first exact-v4 LR-H1 failure remains provenance;
