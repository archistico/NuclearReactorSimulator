...
**Active candidate: M10 Final Replacement-Long Closure Plan 1 — P1 ASYMPTOTIC FIRST-STAGE QUALIFICATION CANDIDATE.**
...
P1 is now the only active implementation. It calibrates stationary slope bands from an unchanged exact-v9 5 MWe reference, then runs exact-v9 5→5.5 MWe, exact-v9 5→6 MWe and historical exact-v4 5→6 MWe asymptotic holds. The primary horizon is 900 s after the load command. Only exact-v9 6 MWe may continue, inside the same P1 gate, to at most 1,800 s when its 900 s tail is still monotonically converging above the reference-derived noise band.

Final physical classifications are `CONVERGED`, `BIASED-STATIONARY` or `INCONCLUSIVE`; `STILL-CONVERGING` is only an intermediate 900 s continuation trigger. Returned P1 evidence must go through P2 before any P3-W/P3-R implementation. An inconclusive P1 is a planning stop.

No `src/`, replacement workload, authority policy, generator-load semantics, protection semantics, exact-v9 or mission @3 change is authorized by P1. A second replacement-long baseline remains forbidden until P4 short 5→10→5 qualification passes.
...
```bat
scripts\run-m10-final-replacement-long-closure-plan1-p1-asymptotic-qualification.cmd
```

The runner first executes `eng\ci-ordinary.cmd` in Release, then the focused explicit P1 gate. Return the complete `artifacts\m10-final-replacement-long-closure-plan1-p1` folder before P2 branch selection or any workload/runtime change.
