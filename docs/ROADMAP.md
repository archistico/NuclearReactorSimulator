...
M10 is **IN PROGRESS**. M10.1–M10.9.4 and the cumulative M10.9.4.1-H.28 continuation are VALIDATED; Phase G is complete and standard production remains explicit at 10 ms. H.19–H.27 provide the numerical/authority/ownership/replay/long-horizon/protection/rollback/off-design provenance. H.28 is now validated after the H.28.1 optimization branch with median wall ratio 4.6215 <= 8, p95 ratio 10.6844 <= 12, allocation ratio 1.1164 <= 16, 379/379 soak commits and zero rollback/fallback/unsafe/untargeted-disagreement violations. Its classification remains `bounded-but-costly`. The current candidate is the single post-optimization H.24 long-horizon requalification required before H.29. Phase I remains deferred under the H.24–H.30 completion roadmap.

H.18 Hotfix 1 validated the four-node split at 261/261. H.19 restored and passed the complete long-horizon/cross-profile 3,046/92/473 qualification at 473/473. H.20 validated fail-closed authority/rollback. H.21 Hotfix 1 validated the real orchestrator sidecar seam without corrected-state commitment. H.22 validated the first separately opt-in corrected-state ownership seam. H.23 Hotfix 2 validated replay/checkpoint and reverse-power protection interaction. H.24–H.27 qualified long-horizon, protection, rollback and off-design operation. H.28 now qualifies performance/soak as `bounded-but-costly`; the current task is the one post-optimization H.24 regression required before H.29. Default current-v2 remains explicit.
...
### M10.9.4.1 Operational Envelope & Numerical Hardening — IN PROGRESS (validated through H.28; corrected path `bounded-but-costly`; current candidate is post-H.28 H.24 long-horizon requalification)
...
6. **H.28.1 optimization branch — CLOSED / validated through green H.28:** conservative performance work preserved the numerical contract. Because committed-runtime implementation changed, rerun H.24 once now after stabilization and before H.29.
