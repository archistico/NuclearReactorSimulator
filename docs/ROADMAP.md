...
M10 is **IN PROGRESS**. M10.1–M10.9.4 and the cumulative M10.9.4.1-H.18 Hotfix 1 continuation are VALIDATED; Phase G is complete and production remains explicit at 10 ms. H.17 Hotfix 6 validated the 30,000-interval/four-profile diagnostic gate: 3,046 P060/F040 trigger intervals, 92 trigger episodes, 473 deterministic representatives, 228/473 three-node convergence and 245 failures. H.18 Hotfix 1 extended only the shadow target set to `steam|stop-out|header|turbine-inlet` and passed 261/261 samples: all 245 H.17 failures recovered, all 16 controls preserved, committed selection transparent and no residual failure remained. H.19 is the current candidate: reproduce the exact H.17 long-horizon census/stratification and qualify the unchanged four-node policy over all 473 representatives before any activation design. Phase I remains deferred.

H.18 Hotfix 1 is validated four-node split evidence, not a production activation: the four-node shadow policy converged 261/261 on all 245 H.17 failures plus 16 success controls. H.19 intentionally restores the expensive complete H.17 census/stratification to prove that the same four-node result holds across all 473 validated representatives without changing the sampling contract.
...
### M10.9.4.1 Operational Envelope & Numerical Hardening — IN PROGRESS (validated through H.18 Hotfix 1; H.19 four-node long-horizon qualification candidate)
