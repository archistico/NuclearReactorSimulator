...
M10 is **IN PROGRESS**. M10.1–M10.9.4 and the cumulative M10.9.4.1-H.27 Hotfix 1 continuation are VALIDATED; Phase G is complete and standard production remains explicit at 10 ms. H.19 qualified the exact four-node policy at 473/473 over the complete 3,046-trigger / 92-episode / 473-representative contract. H.20 validated fail-closed eligibility/rollback. H.21 Hotfix 1 validated real-orchestrator sidecar wiring. H.22 validated the first separately opt-in corrected-candidate commit seam with 443/443 H.20-eligible/H.22-authorized committed corrections and zero unsafe/fallback-commit violations. H.23 Hotfix 2 validated replay/checkpoint/protection. H.24 Hotfix 1 validated 30,008 committed runtime steps and 9,626 corrected commits over the nominal long-horizon domain. H.25 validated the targeted protection/transient matrix. H.26 Hotfix 1 validated 12/12 same-step explicit fallbacks across all eight typed rollback reasons plus four denial controls with zero corrected/partial commits. H.27 Hotfix 1 validated the six-scenario bounded off-design envelope with 2,080 steps, 529 corrected commits, four corrected-qualified and two protected-boundary outcomes. H.28 then failed only the performance/cost ceiling as `unbounded-regression` while numerical safety and determinism remained green. H.28.1-A Hotfix 2 is validated diagnostic evidence; H.28.1-C Hotfix 2 is validated allocation/hot-path optimization evidence; H.28.1-B predictor reuse is validated; H.28.1-D is the current probe CPU optimization candidate and does not rerun the 4h31m55s H.24 gate. Phase I remains deferred under the H.24–H.30 completion roadmap.

H.18 Hotfix 1 validated the four-node split at 261/261. H.19 restored and passed the complete long-horizon/cross-profile 3,046/92/473 qualification at 473/473. H.20 validated fail-closed authority/rollback. H.21 Hotfix 1 validated the real orchestrator sidecar seam without corrected-state commitment. H.22 validated the first separately opt-in corrected-state ownership seam. H.23 Hotfix 2 validated replay/checkpoint and reverse-power protection interaction for that committed opt-in trajectory. H.24 Hotfix 1 has qualified committed long-horizon/cross-profile operation. H.25 has qualified the targeted protection/operational-transient matrix. H.26 qualified integrated rollback/fail-closed fallback. H.27 qualified the bounded off-design envelope. H.28 did not qualify performance, so H.28.1-A now attributes cost before any conservative optimization; default current-v2 remains explicit.
...
### M10.9.4.1 Operational Envelope & Numerical Hardening — IN PROGRESS (validated numerical envelope through H.27 Hotfix 1; H.28 performance failed; H.28.1-A and H.28.1-C validated; H.28.1-B validated predictor reuse; H.28.1-D CPU-hot-path candidate)
...
5. **H.28 — Performance, Cost & Long-Running Operational Soak — FAILED PERFORMANCE GATE:** numerical safety stayed green but cost classified `unbounded-regression`.
6. **H.28.1-A/B/C/D — Attribution then conservative optimization: H.28.1-A validated attribution; H.28.1-C validated Jacobian/probe allocation optimization (~97.6% allocation reduction); H.28.1-B validated historical-explicit predictor reuse; H.28.1-D current exact probe CPU hot-path optimization;** reduce only evidence-backed duplicate work/hot paths, then rerun H.28. Because runtime implementation changes, rerun H.24 once after the optimization branch stabilizes and before H.29, rather than chaining the 4h31m55s gate to every iteration.
7. **H.29 — Production Activation Candidate:** separately review configuration/default selection, kill/fallback path, telemetry and compatibility without inventing another numerical algorithm.
8. **H.30 — Phase H Closure & Production Qualification Decision:** choose evidence-derived `ACTIVATE`, `OPT-IN ONLY` or `REMAIN EXPLICIT`.
