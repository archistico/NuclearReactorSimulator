...
- The first M10 final LR-H1 healthy exact-v4 long-soak acceptance execution exposed a real fail-closed thermodynamic-domain exit at fluid node `outlet` (`v=0.0026153411609661885 m^3/kg`, `u=1615124.4119888516 J/kg`) after the 300 s checkpoint and before 600 s. Diagnostic 1 proves a real pre-failure primary inventory redistribution inside 0..300 s: outlet mass falls from about 7,504.6 kg to 4,605.0 kg, final-60 s `dm/dt` is about -7.914 kg/s and total node mass slope closes to numerical zero. Diagnostic 2 then measures mean final-60 s `channel-return` residual `-7.91379 kg/s`, essentially identical to outlet `dm/dt`; the primary flow controller is Manual/100% with zero integral slope, while the level controller reacts to the redistribution. The remaining limitation is therefore the authored whole-plant/reference operating-point seed rather than a demonstrated controller-driven drift. Diagnostic 3 then probes a distinct exact-v5 260 kg/s instantaneous hydraulic reference point for 600 s without changing exact-v4 or production selection. The run survives, and late branch continuity improves to about -0.178 kg/s, but exact-v5 is not qualified: drum level reaches ~0.9567 and keeps rising, pressure continues falling by about 1 kPa/s, and fuel/structure remain in thermal decline. Diagnostic 4 then confirms that the model remains conservative: the correct M4.4 drum balance uses the internal feedwater pump, and ~98.4% of the late drum accumulation comes from feedwater exceeding separated steam, while late net external power (~-2.477 MW) matches stored-energy decline with microscopic closure residual. Diagnostic 5 completed the whole-cycle census. Its evidence also corrects the earlier 260 kg/s interpretation: exact-v5 did not omit pump internal resistance; it relied on a non-stationary suction/drum pressure separation. Diagnostic 6 therefore authors exact-v6 from the closed equations with `P_suction=P_drum`, yielding a 100 kg/s primary point plus analytically matched steam-path, condenser, feedwater and 32.484 MW thermal state. Exact-v6 remains candidate-only until its 600 s evidence is returned. The envelope and frozen long tolerances must not be widened. See `M10_FINAL_LONG_FAILURE_DIAGNOSTIC6.md`.
