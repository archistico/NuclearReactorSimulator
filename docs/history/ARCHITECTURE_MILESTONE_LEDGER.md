# Architecture milestone ledger

This document preserves the former milestone-led portion of `docs/ARCHITECTURE.md` for engineering provenance. It is **historical**, not a source of current project status and not the preferred way to discover current subsystem ownership.

For current validation/restart state use [`../PROJECT.md`](../PROJECT.md). For stable architecture and ownership boundaries use [`../ARCHITECTURE.md`](../ARCHITECTURE.md).

## Frozen former architecture checkpoint

The following block is retained exactly as historical provenance from the pre-reorganization architecture file. It is intentionally stale and must not be used as current status.
...
## Milestone-led architecture ledger

The sections below preserve the former chronological architecture additions. Later stable/current ownership is consolidated by subsystem in `docs/ARCHITECTURE.md`; chronology remains here so ADR/milestone provenance is not lost.
...
M9.1 is an Application/Infrastructure observability and reconstruction layer. `ScenarioRecorder` subscribes synchronously to `DeterministicStepCompleted` plus the accepted-action journal and retains one immutable `ControlRoomSnapshot` frame per logical step. It may derive recorder events from committed alarm/fault/protection presentation state but must never feed those events back into simulation. "Observational" means no plant-state or command-authority ownership; it does not mean zero execution cost or that evidence-production exceptions are impossible. Recorder failure policy and memory/performance hardening are owned by M11.3.
...
M10.9.7.4 must add a populated golden anchor for `ControlRoomSnapshotFingerprint` v1 before extending archive/timeline equivalence. A future algorithm version is a compatibility event owned by M11.2, not a silent change to `sha256-control-room-snapshot-v1`. The M10.9.7.3 combined `RecentEvents` last-N surface remains an at-a-glance presentation feed; the M10.9.7.4 timeline must separately retain a bounded mission lifecycle spine so dense operational evidence cannot erase activation/terminal/reset narrative. See `APPLICATION_RECORDING_REPLAY_REVIEW.md` and ADR-0184.
...
## Post-Phase-I hydraulic / thermodynamic correction ownership

The authoritative exact-@4 desktop remains a fixed-10-ms corrected-commit system. Two thermodynamic branch-selection semantics intentionally coexist at different ownership boundaries:

- `SimplifiedWaterSteamThermodynamicModel` is the base inverse closure and remains memoryless in its ordinary production branch selection;
- the four-node corrected path may wrap that closure with `ThermodynamicBranchContinuityModel` using the validated bounded previous-phase policy on targeted nodes. When the corrected-candidate supervisor authorizes commit, the candidate produced through that wrapper may become canonical committed plant state.

Therefore `ThermodynamicBranchContinuityModel` must not be understood as globally production-replacing, but neither is the old historical label “shadow-only” sufficient anymore. It is a **conditional corrected-path numerical/thermodynamic owner** whose effect is gated by trigger, eligibility, authority and commit/rollback contracts. The base resolver is not silently changed.

The H.9 Jacobian is built from normalized coordinate residuals and coordinate scales before diagonal regularization and linear solve. Numerical constants in that path must therefore be interpreted in normalized coordinates, not as raw pressure/flow/energy-unit thresholds. The reported pivot-spread estimate is a deterministic rejection diagnostic, not a mathematical condition number.

The reduced quadratic pipe law and ideal one-way-device transitions are intentionally retained through M10/M11 presentation work. Near-zero constitutive regularity, valve/check behavior, stronger conditioning diagnostics and any trajectory-changing summation convention are owned by the post-release M12.2 evidence gate described in `SIMULATION_NUMERICAL_REGULARITY_AND_RUNTIME_REVIEW.md` and ADR-0183.



## Desktop host failure and session-write boundary

The Avalonia desktop host schedules bounded deterministic batches through `DesktopControlRoomRuntimePump`/`ControlRoomRuntimeCoordinator`. The UI timer is a scheduling mechanism, not a physical clock owner. Expected deterministic numerical step failures must be converted at the desktop host boundary into a paused/operator-visible failure state rather than escaping the dispatcher callback. This containment must remain selective: programming defects are not silently swallowed.

Session persistence has a separate data-integrity rule: an existing valid archive is not disposable merely because the operator selected it as the destination of a new save. Desktop local-filesystem overwrite must use complete temporary-write + close/flush + safe replace/move semantics; truncate-first overwrite is not an accepted architecture. Provider abstractions that cannot guarantee the required replacement semantics fail closed rather than silently weakening the contract.

The current host executes Simulation and major presentation projections synchronously on the Avalonia UI thread. This is a known responsiveness coupling, not concurrent timer execution. M11.3 owns measurement and any future single-owner worker/immutable-snapshot handoff. M13 owns structural ViewModel decomposition and stable command-target selection. See `DESKTOP_HOST_FAILURE_AND_SESSION_SAVE_INTEGRITY_REVIEW.md` and ADR-0185.
