...
echo Running M10.9.4.1-H.19 four-node long-horizon and cross-profile shadow qualification...
...
set "REPORT_DIR=%CD%\artifacts\h19-four-node-long-horizon-cross-profile-qualification"
if exist "%REPORT_DIR%" rmdir /s /q "%REPORT_DIR%"

echo H.19 heartbeat will be written to:
echo   "%REPORT_DIR%\00-progress.txt"
echo If the long gate appears quiet, inspect that file from another console.
echo.
...
dotnet test --project "tests\NuclearReactorSimulator.Application.Tests\NuclearReactorSimulator.Application.Tests.csproj" --no-build -- ^
    --explicit only ^
    --filter-trait "Category=FourNodeLongHorizonCrossProfileQualificationAudit" ^
    --parallel none
if errorlevel 1 exit /b 1
...
for %%F in (
    ...
    if not exist "%REPORT_DIR%\%%~F" (
        echo ERROR: expected H.19 audit artifact missing: %%~F
        exit /b 2
    )
)

echo.
type "%REPORT_DIR%\01-current-v2-four-node-long-horizon-cross-profile-qualification.summary.txt"
echo Detailed CSV files: "%REPORT_DIR%"
echo M10.9.4.1-H.19 four-node long-horizon and cross-profile shadow qualification audit completed.
exit /b 0
