...
echo Running M10.9.4.1-H.1 fixed-step numerical-stiffness evidence...
...
dotnet test --project "tests\NuclearReactorSimulator.Application.Tests\NuclearReactorSimulator.Application.Tests.csproj" -- ^
    ...
    "NuclearReactorSimulator.Application.Tests.Scenarios.Gameplay.NumericalStiffnessDecisionAuditTests" ^
    --parallel none
if errorlevel 1 exit /b 1

set "REPORT_DIR=%CD%\artifacts\h1-numerical-stiffness"
...
dotnet test --project "tests\NuclearReactorSimulator.Application.Tests\NuclearReactorSimulator.Application.Tests.csproj" --no-build -- ^
    --explicit only ^
    --filter-trait "Category=NumericalStiffnessDecisionAudit" ^
    --parallel none
if errorlevel 1 exit /b 1
...
for %%F in (
    ...
    if not exist "%REPORT_DIR%\%%~F" (
        echo ERROR: expected H.1 audit artifact missing: %%~F
        exit /b 2
    )
)
...
echo.
echo ================================================================================
echo M10.9.4.1-H.1 NUMERICAL STIFFNESS DECISION SUMMARY
echo ================================================================================
type "%REPORT_DIR%\01-current-v2-fixed-step-stiffness-sweep.summary.txt"
echo Detailed CSV files: "%REPORT_DIR%"
echo M10.9.4.1-H.1 fixed-step numerical-stiffness evidence passed.
exit /b 0
