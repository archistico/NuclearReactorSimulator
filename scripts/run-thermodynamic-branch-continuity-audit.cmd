...
echo Running M10.9.4.1-H.13 thermodynamic branch continuity / hysteresis shadow experiment...
...
set "REPORT_DIR=%CD%\artifacts\h13-thermodynamic-branch-continuity"
...
dotnet test --project "tests\NuclearReactorSimulator.Application.Tests\NuclearReactorSimulator.Application.Tests.csproj" --no-build -- ^
    --explicit only ^
    --filter-trait "Category=ThermodynamicBranchContinuityAudit" ^
    --parallel none
if errorlevel 1 exit /b 1

for %%F in (
    "01-current-v2-thermodynamic-branch-continuity.summary.txt"
    "02-triggered-event-policy-comparison.csv"
    "03-branch-continuity-policy-summary.csv"
    "04-branch-continuity-decisions.csv"
) do (
    if not exist "%REPORT_DIR%\%%~F" (
        echo ERROR: expected H.13 audit artifact missing: %%~F
        exit /b 2
    )
)

echo.
type "%REPORT_DIR%\01-current-v2-thermodynamic-branch-continuity.summary.txt"
echo Detailed CSV files: "%REPORT_DIR%"
echo M10.9.4.1-H.13 thermodynamic branch continuity / hysteresis audit passed.
exit /b 0
