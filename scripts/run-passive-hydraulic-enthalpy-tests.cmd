...
echo Running M10.9.4.1-G.2 focused passive-hydraulic enthalpy migration tests...

dotnet test --project "tests\NuclearReactorSimulator.Domain.Tests\NuclearReactorSimulator.Domain.Tests.csproj" -- ^
    --filter-class "NuclearReactorSimulator.Domain.Tests.Physics.Fluids.PipeDefinitionTests" ^
    --parallel none
if errorlevel 1 exit /b 1

dotnet test --project "tests\NuclearReactorSimulator.Simulation.Tests\NuclearReactorSimulator.Simulation.Tests.csproj" -- ^
    --filter-class ^
    "NuclearReactorSimulator.Simulation.Tests.Physics.Fluids.PipeFlowSolverTests" ^
    "NuclearReactorSimulator.Simulation.Tests.Physics.Fluids.ValveFlowSolverTests" ^
...
set "REPORT_DIR=%CD%\artifacts\g2-passive-hydraulic-enthalpy"
...
dotnet test --project "tests\NuclearReactorSimulator.Application.Tests\NuclearReactorSimulator.Application.Tests.csproj" --no-build -- ^
    --explicit only ^
    --filter-trait "Category=PassiveHydraulicEnthalpyMigrationAudit" ^
    --parallel none
if errorlevel 1 exit /b 1
...
for %%F in (
    ...
    if not exist "%REPORT_DIR%\%%~F" (
        echo ERROR: expected G.2 audit artifact missing: %%~F
        exit /b 2
    )
)
...
echo.
echo ================================================================================
echo M10.9.4.1-G.2 PASSIVE HYDRAULIC ENTHALPY MIGRATION SUMMARY
echo ================================================================================
type "%REPORT_DIR%\01-current-v2-passive-enthalpy-and-pump-work-ownership.summary.txt"
echo Detailed CSV files: "%REPORT_DIR%"
echo M10.9.4.1-G.2 focused passive-hydraulic enthalpy migration tests passed.
exit /b 0
