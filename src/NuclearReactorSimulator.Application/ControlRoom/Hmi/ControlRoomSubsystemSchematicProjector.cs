...
public static class ControlRoomSubsystemSchematicProjector
{
    ...
    private static ControlRoomSubsystemSchematicSnapshot ProjectTurbineSecondary(ControlRoomSnapshot snapshot)
    {
        ...
        var nodes = new[]
        {
            Node("steam-in", "MAIN STEAM HEADER", ControlRoomSubsystemSchematicNodeKind.Header, .02, .05, .21, .20,
            ...
            Node("stop", "STOP VALVE", ControlRoomSubsystemSchematicNodeKind.Valve, .27, .05, .19, .20,
            ...
            Node("control", "CONTROL VALVE", ControlRoomSubsystemSchematicNodeKind.Valve, .50, .05, .19, .20,
            ...
            Node("admission", "ADMISSION VALVE", ControlRoomSubsystemSchematicNodeKind.Valve, .73, .05, .24, .20,
            ...
            Node("stage", "TURBINE STAGE GROUP", ControlRoomSubsystemSchematicNodeKind.TurbineStage, .37, .36, .27, .24,
            ...
            Node("rotor", "TURBINE ROTOR / SHAFT", ControlRoomSubsystemSchematicNodeKind.Rotor, .72, .36, .25, .24,
            ...
            Node("condenser", "CONDENSER / HOTWELL", ControlRoomSubsystemSchematicNodeKind.Condenser, .03, .36, .26, .24,
            ...
            Node("cond-pump", "CONDENSATE PUMP", ControlRoomSubsystemSchematicNodeKind.Pump, .18, .72, .21, .20,
            ...
            Node("fw-pump", "FEEDWATER PUMP", ControlRoomSubsystemSchematicNodeKind.Pump, .45, .72, .21, .20,
            ...
            Node("fw-out", "FEEDWATER TO DRUMS", ControlRoomSubsystemSchematicNodeKind.Boundary, .72, .72, .24, .20,
        ...
        var connections = new[]
        {
            Link("steam-stop", "steam-in", "stop", ControlRoomSubsystemSchematicConnectionKind.Steam, "MAIN STEAM", line is null ? "FLOW —" : DisplayValue(line.MassFlow), "→", line?.MassFlow.State ?? ControlRoomVisualState.Unavailable, .25, .15, P(.23,.15), P(.27,.15)),
            Link("stop-control", "stop", "control", ControlRoomSubsystemSchematicConnectionKind.Steam, "STEAM", train is null ? "FLOW —" : DisplayValue(train.AdmissionFlow), "→", train?.AdmissionFlow.State ?? ControlRoomVisualState.Unavailable, .48, .15, P(.46,.15), P(.50,.15)),
            Link("control-admission", "control", "admission", ControlRoomSubsystemSchematicConnectionKind.Steam, "GOVERNED STEAM", train is null ? "FLOW —" : DisplayValue(train.AdmissionFlow), "→", train?.AdmissionFlow.State ?? ControlRoomVisualState.Unavailable, .71, .15, P(.69,.15), P(.73,.15)),
            Link("admission-stage", "admission", "stage", ControlRoomSubsystemSchematicConnectionKind.Steam, "ADMISSION", train is null ? "FLOW —" : DisplayValue(train.AdmissionFlow), train is null ? "P / T —" : $"{DisplayValue(train.TurbineInletPressure)} · {DisplayValue(train.TurbineInletTemperature)}", train?.AdmissionFlow.State ?? ControlRoomVisualState.Unavailable, .67, .31, P(.85,.25), P(.85,.31), P(.505,.31), P(.505,.36)),
            Link("stage-rotor", "stage", "rotor", ControlRoomSubsystemSchematicConnectionKind.Mechanical, "SHAFT TORQUE / POWER", stage is null ? "POWER —" : DisplayValue(stage.ShaftPower), rotor is null ? "SPEED —" : DisplayValue(rotor.Speed), stage?.ShaftPower.State ?? ControlRoomVisualState.Unavailable, .68, .48, P(.64,.48), P(.72,.48)),
            Link("stage-condenser", "stage", "condenser", ControlRoomSubsystemSchematicConnectionKind.Steam, "EXHAUST STEAM", stage is null ? "FLOW —" : DisplayValue(stage.SteamFlow), condenser is null ? "P —" : DisplayValue(condenser.Pressure), stage?.SteamFlow.State ?? ControlRoomVisualState.Unavailable, .33, .50, P(.37,.50), P(.29,.50)),
            Link("condenser-cond", "condenser", "cond-pump", ControlRoomSubsystemSchematicConnectionKind.Condensate, "CONDENSATE", condenser is null ? "FLOW —" : DisplayValue(condenser.CondensationFlow), condenser is null ? "HOTWELL —" : DisplayValue(condenser.HotwellMass), condenser?.CondensationFlow.State ?? ControlRoomVisualState.Unavailable, .22, .66, P(.16,.60), P(.16,.66), P(.285,.66), P(.285,.72)),
            Link("cond-fw", "cond-pump", "fw-pump", ControlRoomSubsystemSchematicConnectionKind.Condensate, "CONDENSATE / INVENTORY", feedwater is null ? "FLOW —" : DisplayValue(feedwater.CondensatePump.MassFlow), feedwater is null ? "MASS —" : DisplayValue(feedwater.FeedwaterInventoryMass), feedwater?.CondensatePump.MassFlow.State ?? ControlRoomVisualState.Unavailable, .42, .82, P(.39,.82), P(.45,.82)),
            Link("fw-out", "fw-pump", "fw-out", ControlRoomSubsystemSchematicConnectionKind.Feedwater, "FEEDWATER", DisplayValue(snapshot.PrimaryCircuit.TotalFeedwaterFlow), feedwater is null ? "T —" : DisplayValue(feedwater.FeedwaterTemperature), snapshot.PrimaryCircuit.TotalFeedwaterFlow.State, .69, .82, P(.66,.82), P(.72,.82)),
        };
