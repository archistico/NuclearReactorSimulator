...
public sealed class PowerManoeuvringStepDefinition
{
    private readonly IReadOnlyList<string> _requiredCheckIds;

    public PowerManoeuvringStepDefinition(
        string stepId,
        int sequence,
        string title,
        string instruction,
        ...
        ControlRoomCommandKind? suggestedOperatorAction = null)
    {
        ...
        {
            throw new ArgumentException("Required check IDs must be unique within a power-manoeuvring step.", nameof(requiredCheckIds));
        }
