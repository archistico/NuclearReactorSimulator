...
public sealed class GridSynchronizationStepDefinition
{
    private readonly IReadOnlyList<string> _requiredCheckIds;

    public GridSynchronizationStepDefinition(
        string stepId,
        int sequence,
        string title,
        string instruction,
        ...
        ControlRoomCommandKind? suggestedOperatorAction = null)
    {
        ...
        {
            throw new ArgumentException("Required check IDs must be unique within a synchronization step.", nameof(requiredCheckIds));
        }
