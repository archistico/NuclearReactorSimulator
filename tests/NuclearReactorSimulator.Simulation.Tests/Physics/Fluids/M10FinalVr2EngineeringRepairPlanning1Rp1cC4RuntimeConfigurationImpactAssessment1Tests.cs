...
/// <summary>
/// RP1C C4 Runtime Configuration Impact Assessment 1. Evidence-only exact-v9 timing under two
/// explicitly controlled runtime profiles that differ only in AMBIENT-UNSET versus EXPLICIT-REFERENCE-ALL-ON. The test records host-scoped comparative evidence only:
/// it does not select C4, recommend a production runtime configuration, mutate C4, or authorize production/runtime repair.
/// </summary>
public sealed class M10FinalVr2EngineeringRepairPlanning1Rp1cC4RuntimeConfigurationImpactAssessment1Tests
{
    private const string OptInEnvironmentVariable = "NRS_M10_FINAL_VR2_REPAIR_PLANNING1_RP1C_C4_RUNTIME_CONFIGURATION_IMPACT_ASSESSMENT1";
    private const string ProfileEnvironmentVariable = "NRS_M10_FINAL_VR2_REPAIR_PLANNING1_RP1C_C4_A2_PROFILE";
    private const string RunIndexEnvironmentVariable = "NRS_M10_FINAL_VR2_REPAIR_PLANNING1_RP1C_C4_A2_RUN_INDEX";
    private const string HostFingerprintEnvironmentVariable = "NRS_A2_HOST_FINGERPRINT_SHA256";
    private const string ArtifactDirectoryRelative = "artifacts/m10-final-physical-reference-vr2-engineering-repair-planning1-rp1c-c4-runtime-configuration-impact-assessment1";
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "M10FinalVr2EngineeringRepairPlanning1Rp1cC4RuntimeConfigurationImpactAssessment1")]
    public void Rp1cC4RuntimeConfigurationImpactAssessment1_MeasuresOneFreshProfileProcess()
    {
        RequireOptIn();
        var profile = GetProfile(Environment.GetEnvironmentVariable(ProfileEnvironmentVariable));
        var runIndex = ParseRunIndex(Environment.GetEnvironmentVariable(RunIndexEnvironmentVariable));
        ValidateRuntimeEnvironment(profile);
        RequireHostFingerprint();
        ...
        var processDirectory = ResetOwnedProcessDirectory(artifactDirectory, profile.Id, runIndex);
        WriteProcessContract(processDirectory, profile, runIndex, candidate, ceilings);
        WriteExactTiming(processDirectory, profile, runIndex, exactRows, samples, ceilings.ResolveMaximumMicroseconds);
        WriteRuntimeContext(
            processDirectory,
            profile,
            runIndex,
        ...
        PerformanceCeilings ceilings)
    {
        File.WriteAllLines(Path.Combine(directory, "01-process-contract.txt"), new[]
        {
            ...
            "runtime-configuration-profile-evidence-only=True",
            "same-physical-host-required=True",
            "host-fingerprint-sha256=" + RequireHostFingerprint(),
        ...
        double strictMaximumMicroseconds)
    {
        ...
        writer.WriteLine("profile_id,run_index,pass_index,row_index,probe_id,logical_step,node_id,elapsed_us,allocated_bytes,resolved,resolution_path,over_100us,over_max_ceiling");
        for (var index = 0; index < samples.Length; index++)
        {
    ...
    private static ProfileSpec GetProfile(string? value) => value switch
    {
        "AMBIENT-UNSET" => new ProfileSpec("AMBIENT-UNSET", null, null, null, null, null),
        "EXPLICIT-REFERENCE-ALL-ON" => new ProfileSpec("EXPLICIT-REFERENCE-ALL-ON", "1", "1", "1", "1", "1"),
        _ => throw new InvalidOperationException("Unknown or missing Runtime Configuration Impact Assessment profile: " + (value ?? "<null>")),
    };

    private static void ValidateRuntimeEnvironment(ProfileSpec profile)
    {
        Assert.Equal(profile.TieredCompilation, Environment.GetEnvironmentVariable("DOTNET_TieredCompilation"));
        Assert.Equal(profile.TieredPgo, Environment.GetEnvironmentVariable("DOTNET_TieredPGO"));
        Assert.Equal(profile.QuickJit, Environment.GetEnvironmentVariable("DOTNET_TC_QuickJit"));
        Assert.Equal(profile.QuickJitForLoops, Environment.GetEnvironmentVariable("DOTNET_TC_QuickJitForLoops"));
        Assert.Equal(profile.ReadyToRun, Environment.GetEnvironmentVariable("DOTNET_ReadyToRun"));
    }
    ...
    private static string RequireHostFingerprint()
    {
        ...
        Assert.All(value, static character => Assert.True(Uri.IsHexDigit(character)));
    ...
    private static string ResetOwnedProcessDirectory(string artifactDirectory, string profileId, int runIndex)
    {
        var profileDirectory = Path.Combine(artifactDirectory, "profile-" + profileId);
        Directory.CreateDirectory(profileDirectory);
        var path = Path.Combine(profileDirectory, "process-" + runIndex.ToString("00", CultureInfo.InvariantCulture));
    ...
    private readonly record struct ProfileSpec(
        string Id,
