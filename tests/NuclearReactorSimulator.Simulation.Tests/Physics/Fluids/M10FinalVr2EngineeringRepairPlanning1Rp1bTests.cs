...
/// <summary>
/// VR2 Engineering Repair Planning 1 / RP1B. Compares three test-only thermodynamic inverse-closure
/// candidates against the immutable RP1A corpus. Candidate qualification is evidence only; this gate
/// does not select, activate or authorize production thermodynamics.
/// </summary>
public sealed class M10FinalVr2EngineeringRepairPlanning1Rp1bTests
{
    private const string OptInEnvironmentVariable = "NRS_M10_FINAL_VR2_REPAIR_PLANNING1_RP1B";
    private const string ArtifactDirectoryRelative = "artifacts/m10-final-physical-reference-vr2-engineering-repair-planning1-rp1b";
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "M10FinalVr2EngineeringRepairPlanning1Rp1b")]
    public void Rp1b_ProducesTestOnlyShadowCandidateMatrixAgainstFrozenRp1aCorpus()
    {
        RequireOptIn();
        ...
        {
            new("B1-PIECEWISE-REDUCED", static () => new Rp1bPiecewiseReducedCandidate()),
            new("C1-TABULATED-SURROGATE", static () => new Rp1bTabulatedReferenceSurrogateCandidate()),
            new("D1-BOUNDED-IF97-SUBSET", static () => new Rp1bBoundedIf97SubsetCandidate()),
        };
        ...
        var evidenceComplete = candidateRuns.Count == 3
            && candidateRuns.Select(static run => run.CandidateId).Distinct(StringComparer.Ordinal).Count() == 3
        ...
        Assert.True(evidenceComplete, "RP1B shadow matrix evidence generation did not meet the frozen observation-only execution contract.");
    }
    ...
    private static void WriteContractAndProvenance(string directory, PerformanceCeilings ceilings)
    {
        ...
        [
            "gate=VR2-ENGINEERING-REPAIR-PLANNING1-RP1B",
            "scope=TEST-ONLY-SHADOW-CANDIDATE-MATRIX",
            ...
            "candidate-b1=B1-PIECEWISE-REDUCED",
            "candidate-c1=C1-TABULATED-SURROGATE",
            "candidate-d1=D1-BOUNDED-IF97-SUBSET",
            "candidate-final-result-retuning-in-place=FORBIDDEN",
    ...
    private static void WriteComplexity(string directory, IReadOnlyList<CandidateRun> runs)
    {
        ...
        foreach (var run in runs)
        {
            ...
            {
                "B1-PIECEWISE-REDUCED" => "region4-table-plus-reduced-liquid-secants-plus-reduced-vapor",
                "C1-TABULATED-SURROGATE" => "structured-region1-region2-region4-reference-table-interpolation",
                "D1-BOUNDED-IF97-SUBSET" => "tabulated-seed-plus-direct-if97-newton-refinement-with-reference-fallback",
                _ => "unknown",
            };
            lines.Add(string.Join(",", new[]
            {
    ...
    private static void WriteCandidateSummary(string directory, IReadOnlyList<CandidateRun> runs)
    {
        ...
        foreach (var run in runs)
        {
            ...
            lines.Add(string.Join(",", new[]
            {
                ...
                B(row.PlanningTargetMet), B(row.DeterministicRepeat), B(row.PerformanceCeilingMet), B(row.Rp1cSelectionEligible),
            }));
        }
    ...
    private static void WriteRp1bSummary(string directory, IReadOnlyList<CandidateRun> runs, bool evidenceComplete)
    {
        ...
        lines.Add("next-action=Return complete RP1B artifact folder for engineering review before RP1C selection gate implementation.");
        File.WriteAllLines(Path.Combine(directory, "09-rp1b-summary.txt"), lines, Utf8WithoutBom);
    }
