...
/// <summary>
/// VR2 Engineering Repair Planning 1 / RP1B Refinement 1. Compares versioned C2 and D2 test-only
/// candidates against the immutable RP1A corpus after the returned B1/C1/D1 matrix. Candidate
/// qualification is evidence only; this gate does not select, activate or authorize production thermodynamics.
/// </summary>
public sealed class M10FinalVr2EngineeringRepairPlanning1Rp1bRefinement1Tests
{
    private const string OptInEnvironmentVariable = "NRS_M10_FINAL_VR2_REPAIR_PLANNING1_RP1B_REFINEMENT1";
    private const string ArtifactDirectoryRelative = "artifacts/m10-final-physical-reference-vr2-engineering-repair-planning1-rp1b-refinement1";
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "M10FinalVr2EngineeringRepairPlanning1Rp1bRefinement1")]
    public void Rp1bRefinement1_ProducesC2D2ShadowEvidenceAgainstFrozenRp1aCorpus()
    {
        RequireOptIn();
        ...
        {
            new("C2-EXTENDED-TABULATED-SURROGATE", static () => new Rp1bExtendedTabulatedReferenceSurrogateCandidate()),
            new("D2-SEAM-COMPLETE-IF97-COMPARATOR", static () => new Rp1bSeamCompleteIf97ComparatorCandidate()),
        };
        ...
        Assert.True(evidenceComplete, "RP1B Refinement 1 C2/D2 evidence generation did not meet the frozen observation-only execution contract.");
    }
    ...
    private static void WriteContractAndProvenance(string directory, PerformanceCeilings ceilings)
    {
        ...
        [
            "gate=VR2-ENGINEERING-REPAIR-PLANNING1-RP1B-REFINEMENT1",
            "scope=TEST-ONLY-C2-D2-SHADOW-REFINEMENT-MATRIX",
            ...
            "candidate-c2=C2-EXTENDED-TABULATED-SURROGATE",
            "candidate-d2=D2-SEAM-COMPLETE-IF97-COMPARATOR",
            "candidate-first-generation-retuning-in-place=FORBIDDEN",
    ...
    private static void WriteComplexity(string directory, IReadOnlyList<CandidateRun> runs)
    {
        ...
        foreach (var run in runs)
        {
            ...
            {
                "C2-EXTENDED-TABULATED-SURROGATE" => "reachability-aware-region4-plus-dense-region1-region2-reference-table-interpolation-and-near-boundary-fallback",
                "D2-SEAM-COMPLETE-IF97-COMPARATOR" => "c2-phase-aware-seed-plus-direct-if97-newton-refinement-with-reference-fallback",
                _ => "unknown",
            };
            lines.Add(string.Join(",", new[]
            {
    ...
    private static void WriteRp1bSummary(string directory, IReadOnlyList<CandidateRun> runs, bool evidenceComplete)
    {
        ...
        lines.Add("next-action=Return complete RP1B Refinement 1 artifact folder for engineering review before RP1C selection gate implementation.");
        File.WriteAllLines(Path.Combine(directory, "09-rp1b-refinement1-summary.txt"), lines, Utf8WithoutBom);
    }
