...
public sealed class ResidualBacktrackingHydraulicCorrectorSolverTests
{
    private static readonly TimeSpan StiffStep = TimeSpan.FromMilliseconds(100d);

    [Fact]
    public void Options_RejectInvalidIterationLineSearchAndToleranceValues()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(0, 1d, 0.5d, 0.01d, 1e-5d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 0d, 0.5d, 0.01d, 1e-5d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 1.1d, 0.5d, 0.01d, 1e-5d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 1d, 0d, 0.01d, 1e-5d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 1d, 1d, 0.01d, 1e-5d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 1d, 0.5d, 0d, 1e-5d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 0.5d, 0.5d, 0.75d, 1e-5d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 1d, 0.5d, 0.01d, 0d, 1e-2d));
        Assert.Throws<ArgumentOutOfRangeException>(() => new ResidualBacktrackingHydraulicCorrectorOptions(8, 1d, 0.5d, 0.01d, 1e-5d, 0d));
    }

    [Fact]
    public void Step_ConvergesAgainstUnrelaxedFixedPointResidualOnStiffPair()
    {
        var state = CreateTwoNodeState(1_001d, 999d);
        var thermodynamics = new LinearCompressibilityModel();
        var historical = new SemiImplicitHydraulicPrototypeSolver(thermodynamics);
        var solver = new ResidualBacktrackingHydraulicCorrectorSolver(thermodynamics);
        var options = ResidualBacktrackingHydraulicCorrectorOptions.H7AuditDefault;
        var frozen = EmptyBalances();

        var explicitStep = historical.StepExplicit(state, StiffStep, frozen);
        var result = solver.Step(state, StiffStep, frozen, options);
        ...
        Assert.True(result.BacktrackingTrialCount >= result.IterationCount - 1);
        Assert.InRange(result.MinimumAcceptedRelaxationFactor, options.MinimumRelaxationFactor, options.InitialRelaxationFactor);
        ...
        for (var index = 1; index < result.Iterations.Count; index++)
        {
            Assert.True(
                ...
                $"Accepted H.7 iterate {index + 1} did not reduce the fixed-point merit residual.");
        }
    }

    [Fact]
    public void Step_DoesNotTreatTinyRelaxedMotionAsFixedPointConvergence()
    {
        var state = CreateTwoNodeState(1_001d, 999d);
        var solver = new ResidualBacktrackingHydraulicCorrectorSolver(new LinearCompressibilityModel());
        var options = new ResidualBacktrackingHydraulicCorrectorOptions(
            maximumIterations: 2,
        ...
        Assert.True(
            ...
            "A tiny relaxed step must not qualify unless the unrelaxed fixed-point residual itself is small.");
    }

    [Fact]
    public void Step_IntegratesCommittedInventoryExactlyOnceFromAcceptedHydraulicBalances()
    {
        var state = CreateTwoNodeState(1_001d, 999d);
        var solver = new ResidualBacktrackingHydraulicCorrectorSolver(new LinearCompressibilityModel());
        ...
        Assert.InRange(result.AppliedHydraulicMassRateClosureResidualKilogramsPerSecond, 0d, 1e-10d);
        Assert.InRange(result.AppliedHydraulicEnergyOwnershipResidualWatts, 0d, 1e-6d);
    ...
    public void Step_IsExactlyDeterministicForTheSameInputs()
    {
        ...
        var left = solver.Step(state, StiffStep, frozen);
        var right = solver.Step(state, StiffStep, frozen);
        ...
        Assert.True(
            left.CandidateState.FluidNodes
                .Select(static node => (node.Id, node.Mass.Kilograms, node.InternalEnergy.Joules, node.Pressure.Pascals))
                .SequenceEqual(right.CandidateState.FluidNodes.Select(static node => (node.Id, node.Mass.Kilograms, node.InternalEnergy.Joules, node.Pressure.Pascals))));
    }
    ...
    private static PlantState CreateTwoNodeState(double massAKilograms, double massBKilograms)
    {
        ...
        var definition = new PlantDefinition(
            "h7-residual-backtracking-stiff-pair",
            new[] { nodeA, nodeB },
            new[] { pipe },
