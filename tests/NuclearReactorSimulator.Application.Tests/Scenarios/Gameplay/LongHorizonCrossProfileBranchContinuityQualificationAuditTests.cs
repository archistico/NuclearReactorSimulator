...
/// <summary>
/// M10.9.4.1-H.17 long-horizon and cross-profile shadow qualification of the validated H.16
/// steam/stop-out/header bounded previous-phase hysteresis policy. Production remains explicit.
/// </summary>
public sealed class LongHorizonCrossProfileBranchContinuityQualificationAuditTests
{
    private static readonly TimeSpan Step = TimeSpan.FromMilliseconds(10d);
    private static readonly Encoding Utf8WithoutBom = new UTF8Encoding(encoderShouldEmitUTF8Identifier: false);
    private static readonly string[] H17TargetNodeIds = { "steam", "stop-out", "header" };
    ...
    public void DeterminismFingerprints_AreCanonicalAcrossEquivalentTraversalOrders()
    {
        ...
        var inverseForward = new InverseBranchScan(new[] { firstInverseRow, secondInverseRow }, Array.Empty<string>());
        var inverseReverse = new InverseBranchScan(new[] { secondInverseRow, firstInverseRow }, Array.Empty<string>());
        Assert.Equal(InverseScanFingerprint(inverseForward), InverseScanFingerprint(inverseReverse));
    }

    [Fact(Explicit = true)]
    [Trait("Category", "LongHorizonCrossProfileBranchContinuityQualificationAudit")]
    public void ThreeNodeBoundedHysteresis_QualifiesLongHorizonAndCrossProfileWithoutDiscoveringAnotherUntargetedLateShadowNode()
    {
        ...
        Assert.All(stratification.Episodes, static episode =>
            Assert.True(episode.SelectedCount > 0, $"Trigger episode {episode.ProfileId}:{episode.EpisodeIndex} was not represented in the H.17 stratified qualification set."));
        ...
        Assert.True(deterministicRepeat, "H.17 cross-profile three-node policy was not exactly deterministic.");
        ...
        Assert.True(committedDeterministic, "H.17 committed target branch observation was not exactly deterministic.");
        Assert.Equal(totalIntervals * H17TargetNodeIds.Length, committedObservation.CommittedPhaseStateChecks);
        ...
        Assert.True(inverseDeterministic, "H.17 inverse-branch discovery scan was not exactly deterministic.");
        ...
        Assert.True(challengeDeterministic, "H.17 inherited hold/release challenges were not exactly deterministic.");
        ...
        SemiImplicitHydraulicPrototypeSolver solver)
    {
        ...
        for (var index = 1; index <= profile.IntervalCount; index++)
        {
            if (ApplyProfileAction(profile.Kind, index, engine, generatorId, coolingTarget))
            {
                ...
                Assert.False(transition.AnyTripActive, $"Unexpected transition-step trip in H.17 profile {profile.Id} before interval {index}.");
            }
            ...
            Assert.False(presentation.AnyTripActive, $"Unexpected reference trip in H.17 profile {profile.Id} interval {index}.");
        ...
        ISecondaryTransientFaultTarget coolingTarget)
    {
        switch (kind)
        {
            ...
            case ProfileKind.CoolingPulse:
                if (intervalIndex == 501)
                {
                    coolingTarget.ActivateCondenserCoolingDegradation("h17-cooling-pulse", "cooling", 0.75d);
                    return true;
                }
                if (intervalIndex == 3_501)
                {
                    coolingTarget.ClearSecondaryTransientFault("h17-cooling-pulse");
                    return true;
                }
                return false;
            case ProfileKind.CombinedLoadCooling:
                ...
                {
                    coolingTarget.ActivateCondenserCoolingDegradation("h17-combined-cooling", "cooling", 0.75d);
                    return true;
                }
                if (intervalIndex == 3_501)
                {
                ...
                {
                    coolingTarget.ClearSecondaryTransientFault("h17-combined-cooling");
                    return true;
                }
                return false;
            default:
        ...
        IReadOnlySet<(string ProfileId, int IntervalIndex)> triggerKeys)
    {
        ...
        foreach (var profile in trajectories)
        {
            ...
            {
                foreach (var nodeId in H17TargetNodeIds)
                {
        ...
        SimplifiedWaterSteamThermodynamicModel productionThermodynamics)
    {
        ...
        foreach (var item in policy.Events)
        {
            ...
            foreach (var candidateNode in item.Result.CandidateState.FluidNodes.OrderBy(static node => node.Id, StringComparer.Ordinal))
            {
                ...
                var targeted = H17TargetNodeIds.Contains(candidateNode.Id, StringComparer.Ordinal);
        ...
        return new InverseBranchScan(rows.ToArray(), untargetedCandidateOnlyLateShadow.Order(StringComparer.Ordinal).ToArray());
    }
    ...
    private static TriggerStratification StratifyTriggerEvents(IReadOnlyList<ProfileBaseline> baselines)
    {
        ...
        if (selected.Count > MaximumStratifiedQualificationEvents)
        {
            throw new InvalidOperationException(
                $"H.17 trigger-episode mandatory representatives require {selected.Count} H.9 solves, above the bounded stratified qualification budget of {MaximumStratifiedQualificationEvents}. Increase episode coalescing only with explicit evidence; do not silently drop mandatory episode/control representatives.");
        }
    ...
    {
        var rows = new List<CommittedObservationRow>(keys.Count * H17TargetNodeIds.Length);
        ...
        bool qualificationPasses)
    {
        ...
        var directory = Path.Combine(root, "artifacts", "h17-long-horizon-cross-profile-branch-continuity");
        ...
        {
            "profile,interval,selection_reasons,h4_primary_converged,h17_converged,line_search_exhausted,pressure_residual,flow_residual_kg_s,normalized_merit,hydraulic_evaluations,branch_decisions,branch_overrides,previous_phase_holds,hysteresis_releases",
        };
        ...
        {
            "profile,interval,node,targeted,candidate_selected_branch,candidate_selected_phase,candidate_multiple_roots,candidate_late_boundary_saturated_shadow,explicit_selected_branch,explicit_selected_phase,explicit_multiple_roots,explicit_late_boundary_saturated_shadow,candidate_only_late_shadow",
        };
        ...
        var recommendation = qualificationPasses
            ? "H.17 recommendation: the unchanged three-node 2%/5 K bounded previous-phase hysteresis policy qualifies the deterministic trigger-episode-stratified sample over the exhaustive 30000-interval P060/F040 census, preserves committed-state transparency and exposes no new untargeted candidate-only late saturated-root shadow node in the qualified representatives. Keep production explicit; before activation, retain this census+stratified qualification contract and rollback/shadow telemetry."
            : "H.17 recommendation: long-horizon/cross-profile stratified qualification is not yet complete. Keep production explicit and inspect the failing representative, trigger episode or newly discovered untargeted candidate-only late-shadow node before changing the policy or designing activation.";
        ...
        {
            "================================================================================",
            "M10.9.4.1-H.17 LONG-HORIZON & CROSS-PROFILE BRANCH-CONTINUITY SHADOW QUALIFICATION SUMMARY",
            "================================================================================",
            "=== 01-current-v2-long-horizon-cross-profile-three-node-bounded-hysteresis ===",
            "Shadow-only long-horizon and cross-profile qualification of the H.16 validated steam/stop-out/header bounded previous-phase hysteresis policy. Production Resolve(), explicit integration, H.9, P060/F040 and the 2%/5 K limits remain unchanged and no shadow candidate is committed.",
            ...
            FormattableString.Invariant($"inverse-qualified-sample-node-scans={inverseScan.Rows.Count}; candidate-late-shadow-nodes={JoinOrNone(inverseScan.Rows.Where(static item => item.CandidateLateShadow).Select(static item => item.NodeId).Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal))}; untargeted-candidate-only-late-shadow-nodes={JoinOrNone(inverseScan.UntargetedCandidateOnlyLateShadowNodeIds)}; no-new-untargeted-late-shadow-in-qualified-sample={inverseScan.UntargetedCandidateOnlyLateShadowNodeIds.Count == 0}; deterministic-repeat={inverseDeterministic};"),
            ...
            FormattableString.Invariant($"max-closure/ownership={policy.MaximumHydraulicMassClosureKilogramsPerSecond:0.000000000}/{policy.MaximumHydraulicEnergyOwnershipResidualWatts:0.000000000}; long-horizon-cross-profile-stratified-shadow-qualification-passes={qualificationPasses}; h17-audit-passes={auditPasses};"),
            "bounded-hysteresis-limits-changed=False; target-node-set-changed=False; production-resolve-order-changed=False; production-previous-state-hysteresis-introduced=False; production-hybrid-active=False; production-fixed-step=10.000 ms; shadow-candidates-committed=False; H9-corrector-replaced=False; plant-network-orchestrator-routing-changed=False; trigger-retuning=False; physical-coefficient-retuning=False; hidden-flow-filtering=False;",
            recommendation,
            FormattableString.Invariant($"Detailed CSV files: \"{directory}\""),
        };

        File.WriteAllLines(Path.Combine(directory, "01-current-v2-long-horizon-cross-profile-branch-continuity.summary.txt"), summary, Utf8WithoutBom);
        ...
        File.WriteAllLines(Path.Combine(directory, "07-long-horizon-cross-profile-metrics.csv"), new[]
        {
            "metric,value",
            ...
            FormattableString.Invariant($"long_horizon_cross_profile_stratified_shadow_qualification_passes,{qualificationPasses}"),
        }, Utf8WithoutBom);
    }
    ...
    {
        var directory = Path.Combine(FindRepositoryRoot(), "artifacts", "h17-long-horizon-cross-profile-branch-continuity");
        Directory.CreateDirectory(directory);
        File.WriteAllText(Path.Combine(directory, "00-progress.txt"), "H.17 Hotfix 4 stratified audit started." + Environment.NewLine, Utf8WithoutBom);
    }
