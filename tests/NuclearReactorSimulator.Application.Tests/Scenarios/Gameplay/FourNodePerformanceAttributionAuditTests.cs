...
/// <summary>
/// M10.9.4.1-H.28.1-A diagnostic attribution of the failed H.28 corrected-path cost. This gate deliberately
/// applies no performance ceiling and performs no optimization. It measures where wall time, allocations and H.9
/// work are spent while preserving the H.27 numerical contract and the failed-H.28 deterministic trajectory.
/// </summary>
public sealed class FourNodePerformanceAttributionAuditTests
{
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "FourNodePerformanceAttributionAudit")]
    public void FailedH28Cost_IsAttributedWithoutChangingNumericalAuthorityOrTrajectory()
    {
        ResetProgress();
        ...
        for (var step = 1; step <= AttributionSteps; step++)
        {
            ...
            Assert.True(
                ...
                $"Missing H.28.1-A attribution for step {step}.");
        ...
        WriteReports(rows, triggered, nonTriggered, determinism);
        WriteProgress("H.28.1-A performance attribution completed");
    }
        ...
        string determinismFingerprint)
    {
        ...
        File.WriteAllLines(Path.Combine(directory, "02-performance-attribution-steps.csv"), csv, Utf8WithoutBom);
        ...
        var summary = new[]
        {
            "=== 01-current-v2-four-node-performance-attribution ===",
            "H.28.1-A is diagnostic only. It starts from the H.27 Hotfix 1 validated baseline, freezes the failed H.28 performance evidence, and instruments the unchanged corrected path without changing trigger, H.9 mathematics, H.20 authority, H.22 commit ownership, physical coefficients or the 10 ms simulated fixed step.",
            ...
            "h28.1a-performance-attribution-passes=True; h28-remains-failed=True; H29-default-activation-blocked=True;",
            "H.28.1-A recommendation: use the measured cost centers to choose conservative implementation optimizations. Do not raise H.28 ceilings, retune numerical thresholds or change H.9 mathematics from attribution evidence alone.",
        };
        File.WriteAllLines(Path.Combine(directory, "01-four-node-performance-attribution.summary.txt"), summary, Utf8WithoutBom);
        ...
        File.WriteAllLines(Path.Combine(directory, "03-performance-attribution-cost-centers.csv"), centersCsv, Utf8WithoutBom);

        File.WriteAllLines(Path.Combine(directory, "04-performance-attribution-metrics.csv"), new[]
        {
            "metric,value",
            ...
            "h28_1a_attribution_passes,True",
        }, Utf8WithoutBom);
    }
    ...
    private static string ReportDirectory()
        => Path.Combine(FindRepositoryRoot(), "artifacts", "h28-1a-four-node-performance-attribution");
    ...
    private static void ResetProgress()
    {
        ...
        Directory.CreateDirectory(directory);
        WriteProgress("H.28.1-A performance attribution started");
    }
