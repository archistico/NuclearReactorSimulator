...
/// <summary>
/// M10 final LR-H1 Diagnostic 10. Diagnostic 9 proved that exact-v7 leaves the non-vapor fraction rejected by
/// VaporMassFractionLimited in turbine-inlet. Exact-v8 preserves the exact-v7 authored state and governor repair,
/// but assigns that rejected mass and its phase-resolved energy to an explicit hotwell moisture-drain owner.
/// Exact-v4 remains production and all historical exact-version admission semantics remain frozen evidence.
/// </summary>
public sealed class M10FinalLongFailureDiagnostic10Tests
{
    private const string OptInEnvironmentVariable = "NRS_M10_FINAL_LONG_DIAGNOSTIC10";
    ...
    [Fact]
    public void ExactV8Candidate_IsDistinctPreservesHistoricalVersionsAndDoesNotSwitchProductionDefault()
    {
        ...
        var registry = new VersionedInitialConditionRegistry(new IVersionedInitialConditionFactory[] { v4, v5, v6, v7, v8 });
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "M10FinalLongDiagnostic10")]
    public void LR_H1_ExactV8_SixHundredSecondMoistureDrainRequalification()
    {
        RequireOptIn();
        ResetDirectory(ReportDirectory());

        var factory = new DesktopSustainedGenerationMoistureDrainCandidateInitialConditionFactory();
        ...
        for (var step = 1; step <= TotalSteps; step++)
        {
            ...
            if (step % 3000 == 0)
            {
                AppendProgress($"LR-H1 diagnostic10 exact-v8 moisture-drain requalification simulated-seconds={step / StepsPerSecond}; logical-step={step}");
            }
        }
    ...
    private static void WriteAggregateTrajectory(IEnumerable<AggregateSample> rows)
    {
        ...
        File.WriteAllLines(Path.Combine(ReportDirectory(), "110-v8-whole-cycle-moisture-drain-trajectory.csv"), lines, Utf8WithoutBom);
    }
    ...
    private static void WriteNodeTrajectory(IEnumerable<NodeSample> rows)
    {
        ...
        File.WriteAllLines(Path.Combine(ReportDirectory(), "111-v8-node-state-trajectory.csv"), lines, Utf8WithoutBom);
    }
    ...
    private static void WriteFinalNodeSlopes(IReadOnlyList<NodeSample> rows)
    {
        ...
        File.WriteAllLines(Path.Combine(ReportDirectory(), "112-v8-final60-node-slopes.csv"), lines, Utf8WithoutBom);
    }
    ...
    private static void WriteOwnerSummary(IReadOnlyList<AggregateSample> rows, int tripSteps, int rollbacks)
    {
        ...
        File.WriteAllLines(Path.Combine(ReportDirectory(), "113-v8-whole-cycle-moisture-drain-summary.txt"), new[]
        {
            "scope=Diagnostic 10 exact-v8 turbine-admission moisture-drain 600 s requalification; Diagnostic 9 PASS proved commanded-minus-effective equals rejected non-vapor mass and matches turbine-inlet dm/dt; exact-v8 is candidate-only; exact-v4 production selector unchanged; replacement long unauthorized;",
            ...
            "inherited-exact-v7-energy-seed=initial fission power 32.4842538718 MW is preserved intentionally; Diagnostic 10 changes phase ownership, so this value is not assumed to remain the exact whole-cycle energy root and must be judged from returned stored-energy/closure evidence;",
            ...
            "decision-rule=qualify exact-v8 only after returned artifacts show rejected non-vapor admission mass is fully owned by the explicit moisture drain, turbine-inlet accumulation is removed rather than displaced, exhaust/hotwell/feedwater/drum inventories remain bounded, governor repair remains effective, approximately 5 MWe operation is stable and both stage/full-cycle energy closure remain conservative; this candidate freezes no new drift tolerance and does not activate production;",
        }, Utf8WithoutBom);
    }
    ...
    private static void RequireOptIn()
    {
        ...
        {
            throw new InvalidOperationException($"Set {OptInEnvironmentVariable}=1 to run M10 final long Diagnostic 10.");
        }
    }

    private static string ReportDirectory()
        => Path.Combine(FindRepositoryRoot(), "artifacts", "m10-final-long-diagnostic10");
    ...
    private static void ResetDirectory(string directory)
    {
        ...
        File.WriteAllText(Path.Combine(directory, "00-progress.txt"), $"M10 FINAL LONG FAILURE DIAGNOSTIC 10 STARTED{Environment.NewLine}", Utf8WithoutBom);
    }
