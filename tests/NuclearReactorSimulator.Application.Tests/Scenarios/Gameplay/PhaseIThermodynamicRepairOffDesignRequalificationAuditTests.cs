...
/// <summary>
/// I.5 REV1 Hotfix 13 repaired-closure off-design requalification stage 3.
/// Reuses the validated H.27 bounded six-scenario envelope through CorrelationConsistentInverseDomain
/// with real corrected-commit ownership. Registered/default production identities remain unchanged.
/// </summary>
public sealed class PhaseIThermodynamicRepairOffDesignRequalificationAuditTests
{
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "PhaseIThermodynamicRepairOffDesignRequalification")]
    public void OptInCommittedRuntime_MapsStagedOffDesignCasesWithoutUnsafeOwnershipOrHiddenRepair()
    {
        ...
        Assert.All(results, static result => Assert.True(result.EvidenceConditionSatisfied, $"I.5 repair Stage 3 scenario '{result.ScenarioId}' did not satisfy its evidence condition: {result.EvidenceCondition}"));
        Assert.All(results, static result => Assert.Contains(result.Rows, static row => row.TriggerObserved));
        Assert.True(commits > 0 || safeFallbacks > 0, "I.5 repair Stage 3 observed neither corrected ownership nor safe fallback.");
    ...
    private static EnvelopeResult RunCoolingDegradation(string scenarioId, double capacityFraction)
    {
        ...
        target.ActivateCondenserCoolingDegradation($"i5-repair-{scenarioId}", "cooling", capacityFraction);
        probe.Advance(CoolingObservationSteps, "degraded-cooling");
        target.ClearSecondaryTransientFault($"i5-repair-{scenarioId}");
    ...
    private static EnvelopeResult RunCoolingLossProtectionAdjacent()
    {
        ...
        var initialPressure = probe.Rows[^1].CondenserPressurePascals;
        var target = (ISecondaryTransientFaultTarget)probe.Engine;
        target.ActivateCondenserCoolingLoss("i5-repair-cooling-loss", "cooling");
    ...
    private static IReadOnlyList<EnvelopeStepRow> RunDeterminismControl()
    {
        ...
        QueueGeneratorLoad(probe.Engine, ControlRoomCommandKind.GeneratorLoadRaise);
        probe.Advance(16, "load-transition");
        ((ISecondaryTransientFaultTarget)probe.Engine).ActivateCondenserCoolingDegradation("i5-repair-determinism-cooling", "cooling", 0.50d);
        ...
        bool passes)
    {
        ...
        {
            "=== 04-i5-thermodynamic-repair-off-design-requalification-stage3 ===",
            "I.5 repair Stage 3 replays the validated H.27 bounded off-design matrix through CorrelationConsistentInverseDomain and real corrected ownership. Safe H.20 fallback and canonical protection action remain valid envelope boundaries; unsafe or fallback commits are not.",
            ...
            FormattableString.Invariant($"thermodynamic-repair-off-design-requalification-passes={passes}; i5-repair-off-design-stage3-passes={passes};"),
            "I.5 repair Stage 3 recommendation: if replay/protection and all six repaired off-design scenarios are green, preserve the bounded envelope and move next to repaired H.28 performance/cost/operational soak. Do not activate a registered production identity yet and do not broaden the envelope by retuning from this gate alone.",
        };
        ...
        File.WriteAllLines(Path.Combine(directory, "04-i5-thermodynamic-repair-off-design-stage3.summary.txt"), summary, Utf8WithoutBom);
        ...
        csv.AddRange(rows.Select(static row => FormattableString.Invariant(
            $"{row.ScenarioId},{row.Phase},{row.StepIndex},{row.PresentationFingerprint},{row.AnyTripActive},{row.RequestedElectricalPowerMegawatts:G17},{row.CondenserPressurePascals:G17},{row.TriggerObserved},{row.H20CandidateEligible},{row.RollbackRequired},{row.CorrectedCommitAuthorized},{row.CorrectedCandidateCommitted},{row.UntargetedBranchDisagreementDetected},{row.ActivationReason},{row.CommitReason},{row.LatchedActions},{row.LatchedFunctions},{row.MassClosureResidualKilograms:G17},{row.EnergyClosureResidualJoules:G17},{row.BalanceMassRateResidualKilogramsPerSecond:G17},{row.BalancePowerResidualWatts:G17}")));
        File.WriteAllLines(Path.Combine(directory, "05-repaired-off-design-step-telemetry.csv"), csv, Utf8WithoutBom);
        ...
        File.WriteAllLines(Path.Combine(directory, "06-repaired-off-design-qualification-envelope.csv"), envelopeCsv, Utf8WithoutBom);

        File.WriteAllLines(Path.Combine(directory, "07-i5-thermodynamic-repair-off-design-stage3-metrics.csv"), new[]
        {
            "metric,value",
            ...
            $"i5_repair_off_design_stage3_passes,{passes}",
        }, Utf8WithoutBom);
    }


    private static string ReportDirectory()
        => Path.Combine(FindRepositoryRoot(), "artifacts", "i5-thermodynamic-repair-off-design-requalification-stage3");
    ...
    private static void ResetProgress()
    {
        ...
        Directory.CreateDirectory(directory);
        WriteProgress("I.5 repair Stage 3 off-design qualification-envelope audit started");
    }
