...
/// <summary>
/// M10.9.4.1-H.28.1-F CPU-tail gate. It starts from validated H.28.1-D, incorporates the measured E
/// exact hydraulic/continuity optimizations, freezes E's remaining p95 failure, and specializes only
/// finite-difference Jacobian probes to compute the coordinate residual actually consumed by Newton.
/// Full pressure/flow fixed-point merit remains unchanged for initial residuals and line-search trials.
/// </summary>
public sealed class FourNodeJacobianProbeCoordinateResidualAuditTests
{
    ...
    private const double MaximumJacobianWallFractionOfH281E = 1.00d;
    private const double MaximumH9WallFractionOfH281E = 1.00d;
    private const double MaximumTriggerEngineWallFractionOfH281E = 1.00d;
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "FourNodeJacobianProbeCoordinateResidualAudit")]
    public void CoordinateOnlyJacobianProbeResidual_ReducesTriggeredTailWithoutChangingNumericalContract()
    {
        ResetProgress();
        ...
        for (var step = 1; step <= AttributionSteps; step++)
        {
            ...
            Assert.True(
                ...
                $"Missing H.28.1-F Jacobian probe attribution for step {step}.");
        ...
        var jacobianWallPasses = averageJacobianMicroseconds <= H281EBaselineJacobianMicroseconds * MaximumJacobianWallFractionOfH281E;
        var h9WallPasses = averageH9Microseconds <= H281EBaselineH9Microseconds * MaximumH9WallFractionOfH281E;
        var triggerEngineWallPasses = averageTriggerEngineMicroseconds <= H281EBaselineTriggerEngineMicroseconds * MaximumTriggerEngineWallFractionOfH281E;
        ...
        Assert.True(componentReusePasses, $"H.28.1-F hydraulic component reuse was too low: {hydraulicComponentReuseFraction:G17}.");
        Assert.True(mappedProbeElisionPasses, $"H.28.1-F Jacobian probes still performed mapped fluid-node fixed-point integration: count={mappedProbeFluidNodeCount}.");
        Assert.True(jacobianWallPasses, $"H.28.1-F Jacobian wall reduction insufficient. E={H281EBaselineJacobianMicroseconds:G17} us; F={averageJacobianMicroseconds:G17} us.");
        Assert.True(h9WallPasses, $"H.28.1-F H.9 wall reduction insufficient. E={H281EBaselineH9Microseconds:G17} us; F={averageH9Microseconds:G17} us.");
        Assert.True(triggerEngineWallPasses, $"H.28.1-F trigger wall reduction insufficient. E={H281EBaselineTriggerEngineMicroseconds:G17} us; F={averageTriggerEngineMicroseconds:G17} us.");
        Assert.True(tailReadyForH28, $"H.28.1-F triggered p95 is not ready for unchanged H.28. H.28 Requalification 1 explicit p95={H28Requalification1ExplicitP95Microseconds:G17} us; unchanged ratio limit={H28P95RatioLimit:G17}; F triggered p95={triggeredP95Microseconds:G17} us; estimated ratio={estimatedH28TriggeredP95Ratio:G17}.");
        Assert.True(jacobianAllocationPasses, "H.28.1-F regressed the measured H.28.1-E Jacobian allocation level.");
        Assert.True(h9AllocationPasses, "H.28.1-F regressed the measured H.28.1-E H.9 allocation level.");
        Assert.True(predictorRegressionPasses, "H.28.1-F regressed the measured H.28.1-E non-trigger predictor cost.");
        Assert.Equal(H28DeterministicFingerprint, determinism);
        WriteProgress("H.28.1-F Jacobian probe coordinate-residual specialization completed");
    }
        ...
        bool passes)
    {
        ...
        File.WriteAllLines(Path.Combine(directory, "02-jacobian-probe-coordinate-residual-steps.csv"), stepCsv, Utf8WithoutBom);
        ...
        var summary = new[]
        {
            "=== 01-current-v2-jacobian-probe-coordinate-residual ===",
            "H.28.1-F starts from validated H.28.1-D, incorporates the measured H.28.1-E exact continuity/component reuse optimizations, and removes mapped thermodynamic fixed-point work only from finite-difference probes because Newton consumes only their coordinate residual. Initial residual and line-search merit remain full fixed-point evaluations.",
            ...
            $"h28.1f-hydraulic-probe-cpu-tail-reduction-passes={passes}; h28-remains-failed=True; H29-default-activation-blocked=True;",
            "H.28.1-F recommendation: rerun unchanged H.28 only if triggered p95 is at or below the frozen 88.3812 ms threshold with identical deterministic trajectory. Do not raise H.28 ceilings, reduce the 32 probes, retune H.9, or alter P060/F040.",
        };
        File.WriteAllLines(Path.Combine(directory, "01-jacobian-probe-coordinate-residual.summary.txt"), summary, Utf8WithoutBom);

        File.WriteAllLines(Path.Combine(directory, "03-jacobian-probe-coordinate-residual-metrics.csv"), new[]
        {
            "metric,value",
    ...
    private static string ReportDirectory()
        => Path.Combine(FindRepositoryRoot(), "artifacts", "h28-1f-jacobian-probe-coordinate-residual");
    ...
    private static void ResetProgress()
    {
        ...
        Directory.CreateDirectory(directory);
        WriteProgress("H.28.1-F Jacobian probe coordinate-residual specialization started");
    }
