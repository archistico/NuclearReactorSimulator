...
/// <summary>
/// M10.9.4.1-H.24 committed long-horizon/cross-profile qualification of the user-validated H.22 corrected-commit
/// ownership path after H.23 replay/checkpoint/protection qualification. Standard current-v2 remains explicit.
/// </summary>
public sealed class FourNodeCommittedLongHorizonCrossProfileQualificationAuditTests
{
    ...
    private static readonly IReadOnlyDictionary<string, string> FrozenH23Fingerprints =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["H23_ValidatedCommittedReplayProtectionTrace.csv"] = "C0F2CC4B1B2C4CBDB64DB3C689FBC00ACE58788A0F5F0A125A60CBDB4B46CC95",
            ["H23_ValidatedCommittedReplayProtectionMetrics.csv"] = "5335D6ACBB65A4443E73DF9032444249851372183217D4792E89371BC2114469",
        };

    [Fact]
    public void FrozenH23Evidence_RetainsValidatedReplayCheckpointProtectionQualification()
    {
        var evidenceDirectory = EvidenceDirectory();
        foreach (var expected in FrozenH23Fingerprints)
        {
            var path = Path.Combine(evidenceDirectory, expected.Key);
            Assert.True(File.Exists(path), $"Frozen H.23 evidence file is missing: {expected.Key}");
            Assert.Equal(expected.Value, CanonicalSha256(path));
        }

        var summary = File.ReadAllText(Path.Combine(evidenceDirectory, "H23_ValidatedCommittedReplayProtectionSummary.txt"));
        Assert.Contains("recorded-steps=701", summary, StringComparison.Ordinal);
        Assert.Contains("corrected-candidates-committed=242", summary, StringComparison.Ordinal);
        Assert.Contains("full-replay-trace-equivalent=True", summary, StringComparison.Ordinal);
        Assert.Contains("checkpoint-prefix-and-continuation-equivalent=True", summary, StringComparison.Ordinal);
        Assert.Contains("deterministic-trace-repeat=True", summary, StringComparison.Ordinal);
        Assert.Contains("generator-trip-final=True", summary, StringComparison.Ordinal);
        Assert.Contains("reverse-power-latched=True", summary, StringComparison.Ordinal);
        Assert.Contains("four-node-committed-replay-checkpoint-protection-qualification-passes=True", summary, StringComparison.Ordinal);
        Assert.Contains("h23-audit-passes=True", summary, StringComparison.Ordinal);
    }

    [Fact(Explicit = true)]
    [Trait("Category", "FourNodeCommittedLongHorizonCrossProfileQualificationAudit")]
    public void OptInCorrectedCommitRuntime_QualifiesCommittedLongHorizonAndCrossProfileOperation()
    {
        ResetProgress();
        ...
        Assert.True(deterministicControlRepeat, "H.24 committed determinism control did not repeat exactly.");
        ...
        Assert.All(profileResults, static result => Assert.True(result.TriggerCount > 0, $"H.24 profile {result.ProfileId} observed no P060/F040 trigger."));
        Assert.All(profileResults, static result => Assert.True(result.CommitCount > 0, $"H.24 profile {result.ProfileId} observed no corrected commit."));
        ...
        bool passes)
    {
        ...
        {
            "=== 01-current-v2-four-node-committed-long-horizon-cross-profile-qualification ===",
            "H.24 keeps the user-validated H.22/H.23 corrected-commit runtime unchanged and qualifies the separately opt-in committed trajectory over the H.19 four-profile long-horizon operational domain. Safe H.20 rollback/fallback is permitted; unsafe or fallback commits are not. Standard current-v2 remains ExplicitCommittedState.",
            ...
            FormattableString.Invariant($"default-current-v2-mode={defaultMode}; opt-in-corrected-commit-mode=FourNodeBranchContinuityCorrectedCommitOptIn; H23-prerequisite-frozen=True; H22-runtime-changed=False; H20-contract-replaced=False; H19-operational-profile-domain-changed=False; H19-target-set-changed=False; H9-tolerances-retuned=False; P060-F040-retuned=False; bounded-hysteresis-limits-changed=False; physical-coefficient-retuning=False;"),
            FormattableString.Invariant($"four-node-committed-long-horizon-cross-profile-qualification-passes={passes}; h24-audit-passes={passes};"),
            "H.24 recommendation: if this gate is green, keep default production explicit and move next to the broader committed protection/operational-transient matrix. H.24 qualifies duration and the four nominal cross-profile trajectories; it does not yet qualify integrated rollback stress, off-design robustness or production-default activation.",
        };
        ...
        File.WriteAllLines(Path.Combine(directory, "01-four-node-committed-long-horizon-cross-profile.summary.txt"), summary, Utf8WithoutBom);
        ...
        csv.AddRange(rows.Select(static row => FormattableString.Invariant(
            $"{row.ProfileId},{row.Interval},{row.RuntimeStep},{row.IsActionTransitionStep},{row.PresentationFingerprint},{row.TriggerObserved},{row.ShadowCorrectionEvaluated},{row.ProposedAuthority},{row.ActivationReason},{row.RollbackRequired},{row.H20CandidateEligible},{row.CorrectedCommitArmEnabled},{row.CorrectedCommitAuthorized},{row.CommitReason},{row.CorrectedCandidateCommitted},{row.UntargetedBranchDisagreementDetected},{row.BranchOverrideCount},{row.PreviousPhaseHoldCount},{row.HysteresisReleaseCount},{row.ShadowIterationCount},{row.ShadowConverged},{row.ShadowLineSearchExhausted},{row.ShadowMaximumRelativePressureResidual:G17},{row.ShadowMaximumAbsoluteFlowResidualKilogramsPerSecond:G17},{row.ShadowMassClosureKilogramsPerSecond:G17},{row.ShadowEnergyOwnershipResidualWatts:G17},{row.MassClosureResidualKilograms:G17},{row.EnergyClosureResidualJoules:G17},{row.BalanceMassRateResidualKilogramsPerSecond:G17},{row.BalancePowerResidualWatts:G17}")));
        File.WriteAllLines(Path.Combine(directory, "02-committed-long-horizon-step-telemetry.csv"), csv, Utf8WithoutBom);
        ...
        File.WriteAllLines(Path.Combine(directory, "03-profile-qualification-metrics.csv"), profileCsv, Utf8WithoutBom);

        File.WriteAllLines(
            Path.Combine(directory, "04-four-node-committed-long-horizon-metrics.csv"),
            new[]
            {
                "metric,value",
                ...
                FormattableString.Invariant($"h24_audit_passes,{passes}"),
            },
            Utf8WithoutBom);
    }
    ...
    private static string ReportDirectory()
        => Path.Combine(FindRepositoryRoot(), "artifacts", "h24-four-node-committed-long-horizon-cross-profile-qualification");
    ...
    private static void ResetProgress()
    {
        ...
        Directory.CreateDirectory(directory);
        WriteProgress("H.24 committed long-horizon/cross-profile qualification started");
    }
