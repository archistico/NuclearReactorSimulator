...
/// <summary>
/// M10.9.4.1 reference-plant scale evidence. E.2 updates the formerly hybrid A.3 evidence to the
/// user-accepted reduced-scale educational contract while preserving the same deterministic derived calculations.
/// </summary>
public sealed class ReferencePlantScaleAuditTests
{
    ...
    public void CurrentV2_ReferencePlantScaleEvidence_IsExplicitAndReproducible()
    {
        ...
        Assert.Equal(10d, evidence.MaximumSynchronizingCorrectionMegawatts, 12);
        Assert.Equal(1d, evidence.MaximumSynchronizingCorrectionFractionOfNameplate, 12);
        Assert.Equal(2d, evidence.MaximumSynchronizingCorrectionMultipleOfRequestedLoad, 12);
        Assert.Equal(2d, evidence.FrequencyDampingAtSynchronizationToleranceMegawatts, 12);

        Assert.Equal(1.5d, evidence.FullLoadDroopReferenceRiseRpm, 12);
