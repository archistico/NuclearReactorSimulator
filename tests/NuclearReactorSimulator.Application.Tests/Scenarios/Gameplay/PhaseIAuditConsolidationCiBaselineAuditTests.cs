...
public sealed class PhaseIAuditConsolidationCiBaselineAuditTests
{
    ...
    public void AuditTierManifest_SeparatesCurrentCiFromHistoricalFrozenProvenance()
    {
        ...
        AssertTier(rows, "H30-closure", "CURRENT-EVIDENCE", ordinaryRequired: true, scheduledDefault: false);
        AssertTier(rows, "I1-compatibility", "CURRENT-EVIDENCE", ordinaryRequired: true, scheduledDefault: false);
    ...
    public void CiWorkflowContract_UsesGlobalJsonAndKeepsLongGatesOutOfOrdinaryCi()
    {
        ...
        Assert.Contains("run-phase-h-closure-production-qualification-decision-audit.cmd", currentEvidenceScript, StringComparison.Ordinal);
        Assert.Contains("run-profile-compatibility-legacy-retirement-inventory-audit.cmd", currentEvidenceScript, StringComparison.Ordinal);
    ...
    public void PhaseIAuditConsolidationCiBaseline_WritesFrozenTieredEngineeringContract()
    {
        ...
        var retirementRows = new[]
        {
            ...
            "ExplicitCommittedState,RETAIN-AUTHORITATIVE,True,True,True,False",
            ...
            "FourNodeBranchContinuityCorrectedCommitOptIn,RETAIN-QUALIFIED-OPT-IN,True,True,True,False",
        };
        File.WriteAllLines(Path.Combine(directory, "03-legacy-mode-retirement-readiness.csv"), retirementRows, Utf8WithoutBom);

        var summary = new[]
        {
            "=== 01-current-v2-phase-i-audit-consolidation-ci-baseline ===",
            "I.2 freezes the validated I.1 compatibility evidence and establishes a tiered audit/CI execution contract without changing plant physics, numerical mathematics, H.30 OPT-IN ONLY policy, exact-version persistence semantics or the 10 ms fixed step.",
            ...
            "ordinary-ci=clean restore|Release build warnings-as-errors|complete ordinary suite|H30 frozen closure|I1 frozen compatibility|I2 consolidation contract;",
            "scheduled-long-ci=gameplay-long|operational-envelope|reference-plant-scale; H24-post-H28-rerun=False; H28-rerun=False;",
            "historical-frozen-not-ci-required=H24-post-H28|H28-performance|H21-shadow-integration|H5-hybrid-shadow; historical-evidence-preserved=True;",
