...
/// <summary>
/// M10.9.4.1-H.5 Hotfix 2 extended shadow-qualification gate. Production current-v2 is explicitly restored to
/// 10 ms ExplicitCommittedState. The audit evaluates the H.4-selected corrector independently against 5 s of committed production intervals, so non-convergent shadow corrections are recorded without becoming authoritative plant state.
/// </summary>
public sealed class HybridProductionIntegrationAuditTests
{
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "HybridProductionIntegrationAudit")]
    public void CurrentV2ExplicitProduction_ShadowQualifiesSelectedHybridWithoutCommittingCorrections()
    {
    ...
    private static IReadOnlyList<HybridConfiguration> BuildCandidateConfigurations()
    {
        return new[]
        {
            new HybridConfiguration(
                ...
                0.060d,
                40d,
                new SemiImplicitHydraulicPrototypeOptions(72, 0.15d, 1e-5d, 1e-2d)),
        };
    }
    ...
    private static IReadOnlyList<ReferenceInterval> BuildReferenceTrajectory(SemiImplicitHydraulicPrototypeSolver solver)
    {
        ...
        for (var index = 0; index < IntervalCount; index++)
        {
            ...
            Assert.False(presentation.AnyTripActive, $"Unexpected reference trip at H.5 shadow interval {index + 1}.");
        ...
        HybridConfiguration configuration)
    {
        ...
        var accumulator = new RunAccumulator(reference.Count);
        var stopwatch = Stopwatch.StartNew();
        var finalState = reference[^1].End;

        foreach (var interval in reference)
        {
            // Shadow qualification is deliberately interval-local: every corrector sees the actual committed
            // production start state and frozen forcing for that exact explicit interval. Its candidate is
            // measured but never becomes the start state of the next production interval.
            var start = interval.Start;
            var result = solver.Step(start, Step, interval.FrozenNonHydraulicBalances, options);
            finalState = result.CandidateState;
        ...
        stopwatch.Stop();
        return new HybridCandidateRun(configuration, accumulator.Build(reference, finalState, stopwatch.Elapsed.TotalSeconds));
    }
    ...
    {
        var directory = Path.Combine(FindRepositoryRoot(), "artifacts", "h5-hybrid-production-integration");
        ...
        File.WriteAllText(Path.Combine(directory, "01-current-v2-shadow-qualification-trajectory.csv"), sweep.ToString(), Utf8WithoutBom);
        ...
        File.WriteAllText(Path.Combine(directory, "02-current-v2-shadow-correction-events.csv"), selectedTrajectory.ToString(), Utf8WithoutBom);
        ...
        File.WriteAllText(Path.Combine(directory, "03-current-v2-shadow-final-candidate.csv"), finalCsv.ToString(), Utf8WithoutBom);
        ...
        {
            "================================================================================",
            "M10.9.4.1-H.5 HOTFIX 2 EXTENDED SHADOW QUALIFICATION SUMMARY",
            "================================================================================",
            "=== 01-current-v2-extended-shadow-qualification ===",
            "Production remains explicit. The H.4-selected deterministic corrector is evaluated independently against each committed interval of a 5 s production trajectory; shadow candidates are never committed and non-convergence cannot become a runtime fallback or host failure.",
            FormattableString.Invariant($"configurations={candidates.Count}; production-shadow-steps={runSelected.Steps.Count}; selected={selected.Configuration.Id}; pressure-trigger={selected.Configuration.PressureTriggerFraction:0.000000}; flow-trigger={selected.Configuration.FlowTriggerKilogramsPerSecond:0.000} kg/s; relaxation={selected.Configuration.CorrectorOptions.RelaxationFactor:0.000}; max-corrector-iterations={selected.Configuration.CorrectorOptions.MaximumIterations};"),
            FormattableString.Invariant($"shadow-corrections={runSelected.CorrectionCount}/{runSelected.Steps.Count}; correction-fraction={runSelected.CorrectionCount / (double)runSelected.Steps.Count:0.000000}; converged-corrections={runSelected.ConvergedCorrectionCount}/{runSelected.CorrectionCount}; average-iterations-all-steps={runSelected.AverageIterationCount:0.000}; maximum-iterations={runSelected.MaximumIterationCount}; deterministic-work-ratio={runSelected.DeterministicWorkRatio:0.000000};"),
            FormattableString.Invariant($"shadow-candidate chatter-ratios vs explicit: pump={SafeRatio(runSelected.MaximumPumpFlowStepChangeKilogramsPerSecond, explicitRun.MaximumPumpFlowStepChangeKilogramsPerSecond):0.000000}; channel={SafeRatio(runSelected.MaximumChannelFlowStepChangeKilogramsPerSecond, explicitRun.MaximumChannelFlowStepChangeKilogramsPerSecond):0.000000}; return={SafeRatio(runSelected.MaximumReturnFlowStepChangeKilogramsPerSecond, explicitRun.MaximumReturnFlowStepChangeKilogramsPerSecond):0.000000}; pressure={SafeRatio(runSelected.MaximumFractionalSubcooledPressureChange, explicitRun.MaximumFractionalSubcooledPressureChange):0.000000};"),
            FormattableString.Invariant($"last-shadow-candidate-relative-gap-vs-explicit: mass={runSelected.MaximumFinalMassRelativeDifference:0.000000000}; energy={runSelected.MaximumFinalEnergyRelativeDifference:0.000000000}; pressure={runSelected.MaximumFinalPressureRelativeDifference:0.000000000};"),
            ...
            FormattableString.Invariant($"wall-seconds-per-simulated-second explicit/shadow={explicitRun.WallSecondsPerSimulatedSecond:0.000000}/{runSelected.WallSecondsPerSimulatedSecond:0.000000}; observational-shadow-cost-ratio={SafeRatio(runSelected.WallSecondsPerSimulatedSecond, explicitRun.WallSecondsPerSimulatedSecond):0.000000}; deterministic-repeat=True;"),
            $"extended-shadow-qualification-passes={activationRecommended}; production-hybrid-active=False; production-fixed-step=10.000 ms; shadow-candidates-committed=False; physical-coefficient-retuning=False; hidden-flow-filtering=False;",
            activationRecommended
                ? "H.5 Hotfix 2 recommendation: the selected corrector remains admissible over this extended committed-state shadow window; production still remains explicit until a separate activation candidate passes free-running gates."
                : "H.5 Hotfix 2 recommendation: do not activate hybrid production coupling; retain explicit 10 ms production and use the recorded shadow non-convergence/iteration evidence to refine the numerical envelope.",
        };
        var summary = string.Join(Environment.NewLine, summaryLines) + Environment.NewLine;
        File.WriteAllText(Path.Combine(directory, "01-current-v2-shadow-qualification.summary.txt"), summary, Utf8WithoutBom);
