...
/// <summary>
/// M10.9.4.1-H.28.1-C conservative H.9 Jacobian/probe hot-path optimization gate. It preserves the validated
/// numerical work count and trajectory while eliminating full PlantState materialization and repeated topology
/// lookup/canonical-copy work inside transient Jacobian probe evaluations.
/// </summary>
public sealed class FourNodeH9JacobianHotPathOptimizationAuditTests
{
    ...
    [Fact(Explicit = true)]
    [Trait("Category", "FourNodeH9JacobianHotPathOptimizationAudit")]
    public void OptimizedH9ProbePath_ReducesJacobianAllocationWithoutChangingNumericalAuthorityOrTrajectory()
    {
        ResetProgress();
        ...
        for (var step = 1; step <= AttributionSteps; step++)
        {
            ...
            Assert.True(
                ...
                $"Missing H.28.1-C attribution for step {step}.");
        ...
        var hotPathOptimizationPasses = jacobianAllocationPasses && h9AllocationPasses && determinismPasses;
        WriteReports(rows, triggered, nonTriggered, determinism, hotPathOptimizationPasses);

        Assert.True(
            jacobianAllocationPasses,
            $"H.28.1-C Jacobian allocation reduction was insufficient. Baseline={H281ABaselineJacobianAllocatedBytes:G17}, actual={averageJacobianAllocatedBytes:G17}.");
        Assert.True(
            h9AllocationPasses,
            $"H.28.1-C H.9 allocation reduction was insufficient. Baseline={H281ABaselineH9AllocatedBytes:G17}, actual={averageH9AllocatedBytes:G17}.");
        Assert.Equal(H28DeterministicFingerprint, determinism);
        WriteProgress("H.28.1-C H.9 Jacobian/probe hot-path optimization completed");
    }
        ...
        bool hotPathOptimizationPasses)
    {
        ...
        var summary = new[]
        {
            "=== 01-current-v2-h9-jacobian-probe-hot-path-optimization ===",
            "H.28.1-C preserves H.9 finite-difference Newton mathematics and the validated H.20/H.22 ownership contract while removing full PlantState materialization, repeated topology lookup and duplicate canonical-copy work from transient Jacobian probe evaluations.",
            ...
            $"h28.1c-jacobian-probe-hot-path-optimization-passes={hotPathOptimizationPasses}; h28-remains-failed=True; H29-default-activation-blocked=True;",
            "H.28.1-C recommendation: if allocation reduction and deterministic trajectory are green, proceed to the remaining performance work (including H.28.1-B predictor reuse if still desired) and rerun H.28 before any H.29 activation candidate.",
        };
        File.WriteAllLines(Path.Combine(directory, "01-h9-jacobian-probe-hot-path-optimization.summary.txt"), summary, Utf8WithoutBom);
        ...
        File.WriteAllLines(Path.Combine(directory, "03-h9-hot-path-cost-centers.csv"), centersCsv, Utf8WithoutBom);

        File.WriteAllLines(Path.Combine(directory, "04-h9-hot-path-optimization-metrics.csv"), new[]
        {
            "metric,value",
            ...
            $"h28_1c_hot_path_optimization_passes,{hotPathOptimizationPasses}",
        }, Utf8WithoutBom);
    }
    ...
    private static string ReportDirectory()
        => Path.Combine(FindRepositoryRoot(), "artifacts", "h28-1c-h9-jacobian-hot-path-optimization");
    ...
    private static void ResetProgress()
    {
        ...
        Directory.CreateDirectory(directory);
        WriteProgress("H.28.1-C H.9 Jacobian/probe hot-path optimization started");
    }
