> **Current development checkpoint:** M10.9.4.1-D.4.1 is a CANDIDATE built on the validated D.4 baseline. It hardens stop-valve travel ownership, deterministic replay/checkpoint restoration and trip-reset travel resumption. D.4 remains the official validated baseline until local build, complete tests and manual TURBINE-station validation are confirmed. E.2 is not yet implemented.

# Nuclear Reactor Simulator
> **D.4.1 candidate on validated D.4:** D.3.2 Hotfix 3 and the operator valve station remain unchanged. The candidate gives the STOP valve its own optional travel-rate contract, adds full replay plus in-flight checkpoint regressions, and verifies canonical trip reset resumes a preserved OPEN request without hidden repair. The runtime generator/grid contract remains pre-E.
...
The current official continuation baseline is **M10.9.4.1-D.4 — Turbine Valve Operator Authority — VALIDATED**. The working source is **M10.9.4.1-D.4.1 — Turbine Valve Replay, Reset & Travel Ownership Hardening — CANDIDATE**.

The underlying **M7, M8 and M9 phase gates remain COMPLETE / VALIDATED**, and M10.1–M10.9.4 remain validated. M10.9.4.1-A–C and D.1–D.4 have been consolidated into the validated D.4 baseline. Validation on 2026-07-25 passed 944 ordinary tests with zero failures and all 17 unique explicit tests. D.4.1 is a narrow hardening candidate: STOP travel is now definition-owned and independently configurable, valve commands are covered by full replay and in-flight checkpoint seek, and reset-after-trip must resume the preserved STOP OPEN request through finite travel. E.1 accepts a future 10 MWe educational target, but the active source remains at the pre-E 1,000 MW / 150 rpm / correction-only generator-grid contract; E.2 remains planned.
...
M3.1–M3.8, M4.1–M4.7, M5.1–M5.7, M6.1–M6.7, M7.1–M7.7, M8.1–M8.7 hotfix 2 and M9.1–M9.7 are validated; the M3–M9 gates are complete. M10.1–M10.9.4 and M10.9.4.1-D.4 are validated. D.4.1 is the current unvalidated hardening candidate; E.1 is an accepted target decision and E.2 remains a future coordinated migration.
...
M9.1 Recorder, Checkpoints & Full Replay through M9.7 Advanced Fidelity Integration Gate are validated and the M9 gate is complete. M10.1–M10.9.4 and M10.9.4.1-D.4 are validated. The working source is the D.4.1 valve hardening candidate and still contains no E.2 scale or bidirectional-coupling implementation.
