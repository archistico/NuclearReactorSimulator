> **M10.9.4.1-H.18 Hotfix 1 candidate:** built only on user-validated H.17 Hotfix 6. H.17 validated the 30,000-interval/four-profile diagnostic infrastructure but the three-node shadow policy converged only 228/473 representatives. H.18 freezes the validated 473-representative evidence, evaluates all 245 H.17 failures plus 16 success controls with the unchanged bounded 2%/5 K policy extended only to `turbine-inlet`, and diagnoses every remaining failure for residual-floor and all-node inverse-branch structure. Production remains explicit at 10 ms.

> **Validated H.17 Hotfix 6 result:** 3,046 P060/F040 trigger intervals, 92 episodes, 473 representatives, 228 converged / 245 exhausted. The all-node scan found `turbine-inlet`; 120 failures have candidate-vs-explicit turbine-inlet phase mismatch and 125 do not. Determinism, committed transparency, hold/release challenges and closure/ownership remain green.
...
The current official continuation baseline is **M10.9.4.1-H.17 Hotfix 6 — Long-Horizon & Cross-Profile Branch-Continuity Diagnostic — VALIDATED**. Phase G is complete. The working source is **M10.9.4.1-H.18 Hotfix 1 — IReadOnlyList Count Compile Fix over Turbine-Inlet Continuity Extension & Residual-Floor Split Diagnosis — CANDIDATE**.
...
The approved post-hardening product direction is documented in `docs/FUTURE_GAMEPLAY_CONTROL_ROOM_AND_ACCIDENT_DIRECTION.md` and ADR 0121–0123. It adds no current Phase H scope: immediate continuation is H.18 turbine-inlet continuity / residual-floor split diagnosis; production activation remains deferred. Phase I remains deferred until Phase H closes with a fully evidenced numerical policy.
...
M3.1–M3.8, M4.1–M4.7, M5.1–M5.7, M6.1–M6.7, M7.1–M7.7, M8.1–M8.7 hotfix 2 and M9.1–M9.7 are validated; the M3–M9 gates are complete. M10.1–M10.9.4 and M10.9.4.1-H.17 Hotfix 6 are validated, Phase G is complete, and H.18 is the current turbine-inlet continuity / residual-floor split candidate; current-v2 production remains explicit.
