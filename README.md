> **Validated baseline — M10.9.4.1-H.18 Hotfix 1:** build, complete ordinary suite and focused audit passed. The unchanged four-node shadow target set `steam|stop-out|header|turbine-inlet` converged 261/261 over all 245 frozen H.17 failures plus 16 deterministic success controls, recovering 120/120 turbine-inlet-mismatch and 125/125 non-mismatch failures with 16/16 controls preserved. Production remains explicit at 10 ms.

> **Working candidate — M10.9.4.1-H.19:** reruns the complete H.17 30,000-interval/four-profile P060/F040 census and requires the exact validated 3,046 triggers / 92 episodes / 473 representative keys before qualifying the unchanged H.18 four-node shadow policy over all 473 representatives. No activation is included.
...
The current official continuation baseline is **M10.9.4.1-H.18 Hotfix 1 — Turbine-Inlet Continuity Extension & Residual-Floor Split Diagnosis — VALIDATED**. Phase G is complete. The working source is **M10.9.4.1-H.19 — Four-Node Long-Horizon & Cross-Profile Qualification — CANDIDATE**.
...
The approved post-hardening product direction is documented in `docs/FUTURE_GAMEPLAY_CONTROL_ROOM_AND_ACCIDENT_DIRECTION.md` and ADR 0121–0123. It adds no current Phase H scope: immediate continuation is H.19 four-node long-horizon/cross-profile qualification; production activation remains deferred. Phase I remains deferred until Phase H closes with a fully evidenced numerical policy.
...
M3.1–M3.8, M4.1–M4.7, M5.1–M5.7, M6.1–M6.7, M7.1–M7.7, M8.1–M8.7 hotfix 2 and M9.1–M9.7 are validated; the M3–M9 gates are complete. M10.1–M10.9.4 and M10.9.4.1-H.18 Hotfix 1 are validated, Phase G is complete, and H.19 is the current four-node long-horizon/cross-profile qualification candidate; current-v2 production remains explicit.
