...
H.28.1-G is now user-validated: build, complete ordinary suite and focused G gate passed. It preserved the exact deterministic trajectory and reduced triggered p95 to 79.7023 ms, below the unchanged H.28 readiness threshold of 88.3812 ms.

H.28 Requalification 2 now reruns the original H.28 paired benchmark and bounded operational soak over that validated optimized runtime. The H.28 benchmark workload and hard ceilings are unchanged: median wall ratio <= 8, p95 wall ratio <= 12 and median allocation ratio <= 16. This package changes no numerical runtime code.
...
> **Validated performance baseline:** M10.9.4.1-H.28.1-G — VALIDATED.
>
> **Previous H.28 evidence:** Requalification 1 failed only on p95.
>
> **Working candidate:** M10.9.4.1-H.28 Requalification 2.
>
> **Default mode:** `ExplicitCommittedState` at 10 ms.
>
> **H.29:** still blocked. If H.28 becomes green, rerun H.24 once over the stabilized runtime before activation review.
...
The current validated performance continuation is **M10.9.4.1-H.28.1-G — Untargeted Branch-Disagreement Scan Fast Path — VALIDATED**. H.28.1-A/C/B/D remain validated evidence in its provenance; H.28.1-E/F remain failed intermediate performance evidence. **H.28 Requalification 2 is the current candidate** and changes no numerical runtime code. Standard production remains `ExplicitCommittedState`; H.29 remains blocked pending green H.28 plus one H.24 long-horizon rerun.

The underlying M7, M8 and M9 phase gates remain complete, and M10.1–M10.9.4 remain validated. F.1–F.3 Hotfix 1, G.1, G.2 Hotfix 2 and G.3 are validated. The user-supplied G.3 audit records 12 remaining non-turbine enthalpy-mode samples, 2.817289248 MW maximum flow work, 8.972520763 MW total absolute flow work and zero maximum ownership residual.

The supplied G.4 audit closes Phase G with 2 current-v2 enthalpy-mode turbine stages, 2.506379668 MW maximum flow work, 5.457103652 MW maximum shaft power and zero ownership residual. Shaft work remains single-owned and turbine work was not retuned.

H.4 is user-validated. It selected `P060-F040-R015` with 2/50 corrections, deterministic work ratio 2.140000, chatter ratios pump/channel/return/pressure 0.432616/0.135885/0.086691/0.921832, observational wall-cost ratio 1.662880, zero conservation/ownership residuals and exact deterministic repeat. H.5 Hotfix 1 attempted direct production activation and failed ordinary validation. H.5 Hotfix 2 is user-validated: production is explicitly restored to 10 ms `ExplicitCommittedState`; over 500 committed shadow intervals P060-F040-R015 triggered 7 corrections, converged 5/7, used deterministic work ratio 1.492000 and observational cost ratio 1.480162, with `extended-shadow-qualification-passes=False`. H.6 is also user-validated: the bounded six-profile Picard rescue sweep selected `R0125-I096`, but converged only 6/7; the two-tier ladder remained 6/7 with deterministic work ratio 1.700000 and `refined-envelope-qualification-passes=False`. H.7 Hotfix 1 is user-validated: the true-residual/backtracking solver converged 5/7, exhausted its line search on two events, used deterministic work ratio 1.308000 and returned `corrector-algorithm-revision-qualification-passes=False`. H.8 is user-validated: safeguarded Anderson also converged 5/7 with two line-search exhaustions, Anderson attempts/acceptances 30/24, residual fallback 13/11, deterministic work ratio 1.212000, exact repeat and `accelerated-corrector-qualification-passes=False`. H.9 is user-validated: conservative-coordinate finite-difference Jacobian plus damped Newton still converged 5/7, exhausted two line searches, built/accepted 23/21 Jacobian directions with zero conditioning rejections, used deterministic work ratio 2.702000 and returned `jacobian-informed-corrector-qualification-passes=False`. H.10 is user-validated: it found no hydraulic branch switching/non-smooth paths but exactly two thermodynamic phase/envelope switching nodes around the persistent H.9 candidates and none at the matching explicit endpoints. H.11 is user-validated: it localized those phase boundaries to `steam` at interval 200 and `stop-out` at interval 360, both on energy+mass probes. H.12 is user-validated: both nodes expose overlapping saturated/superheated roots, coarse saturated detection toggles, five late boundary-aware saturated roots are shadowed by earlier coarse-superheated selection, and `previousState` provides no tie-break. H.13 Hotfix 2 is user-validated: targeted previous-phase continuity and bounded 2%/5 K hysteresis both move unchanged H.9 from 5/7 to 7/7 with zero line-search exhaustion, exact repeat and preserved conservation/ownership; the bounded policy records zero release events in the frozen set. H.14 Hotfix 1 is now user-validated: the broader window contains 15 P060/F040 triggers and bounded hysteresis converges 14/15, with interval 723 the sole line-search exhaustion; all four hold/release challenges pass, and interval 723 records zero branch overrides/releases. H.15 Hotfix 1 is user-validated and localized interval 723 to `header`, confirming the same H.12 inverse-map mechanism outside the original H.13 target set. H.16 is user-validated: the H.14 two-node control remains 14/15, while the unchanged bounded policy at `steam|stop-out|header` reaches 15/15 with zero exhaustion, recovers interval 723 through 68 `header` overrides, preserves exact determinism/closure/ownership, leaves 6,000 committed observations transparent and retains the four hold/release challenge passes. H.17 Hotfix 6 is user-validated diagnostic evidence: over 30,000 intervals and four current-v2 profiles it censused 3,046 P060/F040 trigger intervals, stratified them into 92 episodes and evaluated 473 representatives. Only 228/473 converged under the three-node policy; the all-node scan found a new `turbine-inlet` disagreement class. H.18 Hotfix 1 is user-validated: adding only `turbine-inlet` to the shadow target set recovered all 245 H.17 failures and preserved all 16 controls, for 261/261 convergence with committed transparency and no residual failure. H.19 is user-validated: it reproduced the complete H.17 30,000-interval/four-profile census at 3,046 triggers / 92 episodes / exact 473 representative keys and converged 473/473, recovering 245/245 H.17 failures and preserving 228/228 successes with committed transparency and no new untargeted branch disagreement. H.20 is user-validated: its frozen-evidence contract gave 473/473 default explicit decisions, 473/473 armed shadow eligibility, zero commits and 8/8 typed rollback challenges. H.21 Hotfix 1 is user-validated after its audit-only CS0136 repair: the orchestrator sidecar remained presentation-equivalent 2000/2000, produced 15/15 H.20-eligible candidates with zero rollback/commit/untargeted disagreement and deterministic telemetry. H.22 is user-validated: over 2,000 committed opt-in intervals it observed 443 P060/F040 triggers and committed exactly 443/443 H.20-eligible/H.22-authorized corrected candidates with zero rollback, fallback-commit violations, unsafe commits or untargeted disagreements, exact repeat and tight conservation. H.23 Hotfix 2 is user-validated after the audit-only CS0246 namespace-import repair and descriptor-test case-sensitivity repair: 701 recorded steps, 242 corrected commits, exact full replay/checkpoint continuation, final reverse-power generator trip and zero unsafe/fallback-commit violations. H.24 Hotfix 1 is now user-validated: 30,000 qualification intervals plus 8 action-transition steps, 9,626 corrected commits, zero rollback/fallback-commit/unsafe-commit/untargeted-disagreement violations and all four nominal profiles trip-free. Its focused gate took 4h31m55s and is retained as a rare qualification gate. H.25 is user-validated: its five-scenario targeted protection/transient matrix completed 837 runtime steps in 5m29s with 178 corrected commits and zero rollback/fallback-commit/unsafe-commit violations. H.26 Hotfix 1 is user-validated: 12/12 same-step explicit fallbacks covered all eight typed H.20 rollback reasons plus four denial controls with zero corrected/partial commits and deterministic repeat. The first H.27 focused gate completed the staged matrix but failed only an over-prescriptive `high-load-10mwe` trip-free evidence condition. H.27 Hotfix 1 aligns that condition with envelope mapping: the 10 MWe requested-load point must be observed, while a canonical protection action is a valid `protected-boundary`. H.27 Hotfix 1 is validated. H.28 then failed only its performance ceilings while preserving numerical safety/determinism. H.28.1-A Hotfix 2 validated the performance attribution: H.9 Jacobian/probes dominate triggered cost. H.28.1-C Hotfix 2 removed probe allocation churn; H.28.1-B is validated and removed duplicated predictor work; H.28.1-D now targets the remaining probe CPU duplication without changing H.9 mathematics. H.29 remains blocked and standard current-v2 remains explicit. The load-pulse and combined load leg retain the validated normal 5→0→5 MWe request trajectory.

See `docs/M10_9_4_1_H2_NUMERICAL_METHOD_DECISION.md`, `docs/M10_9_4_1_H2_VALIDATION_CHECKLIST.md`, `docs/M10_9_4_1_H1_NUMERICAL_STIFFNESS_EVIDENCE.md`, `docs/OPERATIONAL_ENVELOPE_NUMERICAL_HARDENING_PLAN.md`, `docs/KNOWN_MODEL_LIMITATIONS.md`, ADR 0125 and ADR 0126.

The approved post-hardening product direction is documented in `docs/FUTURE_GAMEPLAY_CONTROL_ROOM_AND_ACCIDENT_DIRECTION.md` and ADR 0121–0123. It adds no current Phase H scope: immediate continuation is H.27 off-design robustness/qualification-envelope mapping over the validated H.26/H.25/H.24/H.23/H.22 opt-in path. The authoritative Phase H continuation is H.24–H.30: duration/cross-profile, protection/transient matrix, integrated rollback stress, off-design envelope, performance/soak, production activation candidate and final closure decision. Standard/default corrected-state production activation remains deferred. Phase I remains deferred until Phase H closes with a fully evidenced numerical policy.
...
The current lumped law is `Δp = R · m_dot · |m_dot|`. Flow reverses naturally when the endpoint pressure difference reverses. Historical definitions advect upstream specific internal energy; current-v2 definitions now select upstream enthalpy through the Phase G versioned transport mode. Both conventions return exactly conservative endpoint balances. Detailed friction correlations and two-phase transport remain deferred to their dedicated milestones. Valve restriction in M1.4 and active pump pressure in M1.5 both compose over the same hydraulic sign convention and network boundary.
...
M3.1–M3.8, M4.1–M4.7, M5.1–M5.7, M6.1–M6.7, M7.1–M7.7, M8.1–M8.7 hotfix 2 and M9.1–M9.7 are validated; the M3–M9 gates are complete. M10.1–M10.9.4 and M10.9.4.1-H.20 are validated, Phase G is complete, and H.21 is the current opt-in orchestrator shadow-wiring/telemetry candidate; standard current-v2 production remains explicit.
