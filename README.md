> **Validated baseline — M10.9.4.1-H.19:** build, complete ordinary suite and focused audit passed. The complete 30,000-interval/four-profile P060/F040 census reproduced 3,046 triggers / 92 episodes / the exact 473 frozen representative keys; the unchanged four-node `steam|stop-out|header|turbine-inlet` shadow policy converged 473/473, recovered 245/245 H.17 failures, preserved 228/228 successes, remained committed-state transparent and exposed no new untargeted branch disagreement. Production remains explicit at 10 ms.

> **Working candidate — M10.9.4.1-H.20:** freezes the validated H.19 evidence and defines a default-disabled, fail-closed shadow activation/rollback/telemetry contract. The supervisor cannot authorize production commit and is not wired into `PlantNetworkOrchestrator`.

# Nuclear Reactor Simulator
> **Validated H.16 result:** the H.14 two-node control reproduces 14/15 with interval 723 failed, while the unchanged bounded policy extended to `steam|stop-out|header` converges 15/15 with zero line-search exhaustion and 68 `header` overrides at interval 723. All 6,000 committed target observations are transparent and the hold/release challenges pass.
...
The current official continuation baseline is **M10.9.4.1-H.19 — Four-Node Long-Horizon & Cross-Profile Qualification — VALIDATED**. Phase G is complete. The working source is **M10.9.4.1-H.20 — Four-Node Activation Contract, Rollback & Shadow Telemetry — CANDIDATE**.
...
The approved post-hardening product direction is documented in `docs/FUTURE_GAMEPLAY_CONTROL_ROOM_AND_ACCIDENT_DIRECTION.md` and ADR 0121–0123. It adds no current Phase H scope: immediate continuation is H.20 fail-closed activation/rollback/shadow-telemetry contract qualification; production activation remains deferred. Phase I remains deferred until Phase H closes with a fully evidenced numerical policy.
...
M3.1–M3.8, M4.1–M4.7, M5.1–M5.7, M6.1–M6.7, M7.1–M7.7, M8.1–M8.7 hotfix 2 and M9.1–M9.7 are validated; the M3–M9 gates are complete. M10.1–M10.9.4 and M10.9.4.1-H.19 are validated, Phase G is complete, and H.20 is the current fail-closed activation/rollback/shadow-telemetry contract candidate; current-v2 production remains explicit.
...
M9.1 Recorder, Checkpoints & Full Replay through M9.7 Advanced Fidelity Integration Gate are validated and the M9 gate is complete. M10.1–M10.9.4 and M10.9.4.1-H.17 Hotfix 6 are validated and Phase G is complete. H.13–H.16 established the bounded 2%/5 K branch-continuity mechanism at `steam|stop-out|header`; H.17 then validated the 30,000-interval/four-profile diagnostic infrastructure and found a new `turbine-inlet` mismatch class plus a separate non-mismatch failure class. The working source is H.18, which keeps production current-v2 explicit, evaluates all 245 H.17 failures with `turbine-inlet` added only in shadow, and diagnoses any residual fixed-point failure class.
